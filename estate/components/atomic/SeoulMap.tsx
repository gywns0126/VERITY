// SeoulMap — 서울 25구 SVG choropleth + hover 애니메이션
// ATOMIC — LandexMapDashboard·ScoreDetailPanel·WatchGroupsDashboard 등 재사용
// 데이터 소스: southkorea/seoul-maps GeoJSON (simplified). Polygon → SVG path 변환 + centroid.
...
const colors = {
  ...
  accent: {
    estate: "#B8864D",
    estateHover: "#D4A063",
    estateMuted: "rgba(184,134,77,0.30)",
    terminal: "#B5FF19",
  },
  grade: {
    HOT: "#EF4444",
...
const typography = {
  ...
  size: { xs: 11, sm: 13, base: 14, md: 16, lg: 20 },
  weight: { regular: 400, medium: 500, semibold: 600 },
} as const

const spacing = { s1: 4, s2: 8, s3: 12, s4: 16 } as const
const radius = { sm: 4, md: 8, lg: 12 } as const
const motion = { fast: 180, base: 240 } as const
/* ◆ TOKENS END ◆ */
...
type GuScore = {
  score?: number   // LANDEX 종합 (0~100)
  grade?: GradeLabel
  label?: string   // 보조 라벨 (예: "C / COOL")
  v?: number; d?: number; s?: number; c?: number; r?: number
}

type ScoreByGu = Record<string, GuScore>

interface SeoulMapProps {
  scoreByGu?: ScoreByGu
  selectedGu?: string
  onSelect?: (gu: string) => void
  onHover?: (gu: string | null) => void
  showLabels?: boolean
  showTooltip?: boolean
  height?: number
  /** 점수 없을 때 default fill 강도 (0~1) */
  emptyOpacity?: number
}

// score → grade (40 미만은 AVOID/COOL 으로 분기, 80 이상 HOT)
function gradeFromScore(score?: number): GradeLabel | undefined {
  if (score === undefined || score === null || isNaN(score)) return undefined
  if (score >= 80) return "HOT"
  if (score >= 60) return "WARM"
  if (score >= 40) return "NEUT"
  if (score >= 20) return "COOL"
  return "AVOID"
}

function gradeColor(g?: GradeLabel): string {
  if (!g) return colors.text.tertiary
  return colors.grade[g]
}

// trailing "구" 만 제거 (head 가 2글자 이상일 때). 강남구→강남, 구로구→구로, 중구→중구.
...
function SeoulMap({
  ...
  const safeScores: ScoreByGu = scoreByGu ?? {}
  ...
  const tooltipScore = tooltipGu ? safeScores[tooltipGu] : undefined
  ...
  return (
    ...
    >
      ...
      >
        {/* 한강 표현 — 단순 배경 stripe */}
        <rect x="0" y="0" width="1000" height="823" fill={colors.bg.secondary} />

        {/* 25구 path */}
        {SEOUL_PATHS.map(({ name, d, cx, cy }) => {
          const sc = safeScores[name]
          const grade = sc?.grade ?? gradeFromScore(sc?.score)
          const baseFill = gradeColor(grade)
          const hasData = grade !== undefined
          const isHovered = hovered === name
          const isSelected = selectedGu === name

          const fillOpacity = hasData
            ? isHovered ? 0.62 : isSelected ? 0.50 : 0.28
            : isHovered ? 0.20 : isSelected ? 0.16 : emptyOpacity

          const stroke = isSelected
            ? colors.accent.estate
            : isHovered
            ? colors.accent.estateHover
            : colors.border.strong
          const strokeWidth = isSelected ? 2.5 : isHovered ? 1.8 : 0.8

          const filter =
            isHovered || isSelected
              ? `drop-shadow(0 0 6px ${colors.accent.estateMuted})`
              : "none"

          const showValue = (isHovered || isSelected) && sc?.score !== undefined
          const labelY = showValue ? cy - 11 : cy
          // 컬러 채움 위에서도 또렷하게: paint-order 로 어두운 stroke 깔고 흰 fill
          ...
          return (
            <g
              key={name}
              style={{ cursor: "pointer", transition: `filter ${motion.fast}ms ease` }}
            ...
            >
              <path
                ...
                strokeLinejoin="round"
                style={{
                  transition: `fill-opacity ${motion.fast}ms ease, stroke ${motion.fast}ms ease, stroke-width ${motion.fast}ms ease, filter ${motion.fast}ms ease`,
                  filter,
                }}
              />
              {showLabels && (
                <>
                  <text
                    ...
                    fontWeight={isHovered || isSelected ? typography.weight.semibold : typography.weight.medium}
                    fill={hasData ? colors.text.primary : colors.text.tertiary}
                    style={{
                      pointerEvents: "none",
                      transition: `font-size ${motion.fast}ms ease`,
                      ...labelOutline,
                    }}
                  >
      ...
      {showTooltip && tooltipGu && (
        ...
        >
          ...
          {tooltipScore?.score !== undefined && (
            ...
            >
              LANDEX {tooltipScore.score.toFixed(1)}
              {tooltipScore.grade && (
                <span
                  style={{
                    marginLeft: spacing.s2,
                    color: gradeColor(tooltipScore.grade),
                    fontWeight: typography.weight.semibold,
                  }}
                >
                  {tooltipScore.grade}
          ...
          <span key={g} style={{ display: "inline-flex", alignItems: "center", gap: 6 }}>
            <span
              style={{
                ...
                opacity: 0.6,
                border: `1px solid ${colors.grade[g]}`,
              }}
            />
            {g}
          </span>
...
addPropertyControls(SeoulMap, {
  scoreByGu: {
    type: ControlType.Object,
    title: "Score by Gu",
    controls: {},
    defaultValue: {
      강남구: { score: 39.7, grade: "COOL" },
      도봉구: { score: 70.4, grade: "WARM" },
      서초구: { score: 65.2, grade: "WARM" },
      송파구: { score: 58.0, grade: "NEUT" },
    },
  },
  selectedGu: {
    type: ControlType.String,
    defaultValue: "",
    description: "선택된 구 — 예: 강남구",
  },
  showLabels: { type: ControlType.Boolean, defaultValue: true },
  showTooltip: { type: ControlType.Boolean, defaultValue: true },
  height: { type: ControlType.Number, defaultValue: 480, min: 240, max: 1200, step: 20 },
  emptyOpacity: {
