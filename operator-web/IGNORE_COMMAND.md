...
N=200                              # 실측 ~330 커밋/일 → ~14.5h 창 (봇 데이터 커밋 흡수)
HEAD~N 없으면 N=1, 그것도 없으면 exit 1 (fail-open: 배포 누락 ≫ 중복)
git diff --quiet HEAD~N HEAD -- operator-web/  → 변경 없으면 SKIP / 있으면 BUILD
```

## 불변식 (변경 시 회귀)
- 경로 스코프 `-- operator-web/` (봇 데이터 커밋에 반응 금지)
- N ≥ 200 (하향 금지; 실측 커밋 속도 기준)
