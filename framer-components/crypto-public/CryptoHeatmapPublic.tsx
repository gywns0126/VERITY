...
const LIGHT = {
    ...
    line: "#f0f1f3", up: "#15c47e", down: "#f04452", warn: "#ff9500", accent: "#0ca678", tileInk: "#191f28",
}
const DARK = {
    bg: "#0f1318", card: "#171c23", ink: "#e3e7ec", sub: "#9aa4b1", faint: "#828d9b",
    line: "#222730", up: "#3ddc97", down: "#ff6b76", warn: "#ffb454", accent: "#3ddc97", tileInk: "#ffffff",
}
...
const DEMO = {
    coins: [
        { id: "bitcoin", symbol: "BTC", name: "Bitcoin", market_cap: 1.29e12, current_price: 64663, change_pct: 0.72, change_pct_7d: -3.8, fdv_mc_ratio: 1.0, circulating_ratio: 95.5 },
        { id: "ethereum", symbol: "ETH", name: "Ethereum", market_cap: 2.1e11, current_price: 2630, change_pct: 0.67, change_pct_7d: -5.1, fdv_mc_ratio: 1.0, circulating_ratio: 100 },
        { id: "tether", symbol: "USDT", name: "Tether", market_cap: 1.86e11, current_price: 1, change_pct: 0.0, change_pct_7d: 0.01, fdv_mc_ratio: 1.0, circulating_ratio: 100 },
        { id: "hyperliquid", symbol: "HYPE", name: "Hyperliquid", market_cap: 8.0e10, current_price: 24, change_pct: 0.8, change_pct_7d: 2.1, fdv_mc_ratio: 4.3, circulating_ratio: 22.2 },
        { id: "solana", symbol: "SOL", name: "Solana", market_cap: 6.5e10, current_price: 140, change_pct: -1.2, change_pct_7d: 4.3, fdv_mc_ratio: 1.2, circulating_ratio: 78 },
        { id: "ripple", symbol: "XRP", name: "XRP", market_cap: 3.1e10, current_price: 0.52, change_pct: 1.9, change_pct_7d: -2.0, fdv_mc_ratio: 1.8, circulating_ratio: 56 },
    ],
}
...
export default function CryptoHeatmapPublic(props: { dataUrl?: string; dark?: boolean; metric?: Metric }) {
    ...
    const wrap: CSSProperties = { width: "100%", boxSizing: "border-box", fontFamily: FONT, color: C.ink, background: C.bg, borderRadius: 18, padding: 18, display: "flex", flexDirection: "column", gap: 12, position: "relative" }

    if (!data) {
        const skBase = isDark ? "#1e242c" : "#e9edf1"
        const skHi = isDark ? "#2a313b" : "#f3f5f7"
        const W = [30, 18, 16, 12, 14, 10, 12, 8, 10, 8, 9, 7, 8, 6, 9, 7, 6, 8, 6, 7]
        return (
            <div ref={rootRef} style={wrap}>
                <style>{`@keyframes vshShimmer{0%{background-position:-400px 0}100%{background-position:400px 0}}`}</style>
                <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                    {W.map((w, i) => (
                        <div key={i} style={{ flexBasis: `calc(${w}% - 3px)`, flexGrow: w, minWidth: 56, height: 80, borderRadius: 6, background: skBase, backgroundImage: `linear-gradient(90deg, ${skBase} 25%, ${skHi} 37%, ${skBase} 63%)`, backgroundSize: "800px 100%", animation: "vshShimmer 1.4s ease-in-out infinite" }} />
                    ))}
                </div>
            </div>
        )
    ...
    return (
        <div ref={rootRef} style={wrap} onMouseLeave={() => setHover(null)}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                <span style={{ ...MONO, fontSize: 11.5, color: C.faint, fontWeight: 700 }}>{coins.length}개 · 박스=시총 · {metric === "dilution" ? "색=희석 overhang" : "색=" + metric + " 변동"}</span>
                {metric === "dilution" ? (
            ...
            {hover && (() => {
                ...
                return (
                    <div style={{ position: "fixed", left, top, width: W, zIndex: 9999, background: C.card, borderRadius: 12, padding: "11px 13px", boxShadow: "0 8px 26px rgba(0,0,0,0.22)", pointerEvents: "none", fontFamily: FONT }}>
...
addPropertyControls(CryptoHeatmapPublic, {
