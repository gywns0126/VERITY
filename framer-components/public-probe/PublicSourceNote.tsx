...
export default function PublicSourceNote(props: {
    ...
    const [ticker, setTicker] = useState(() => onCanvas ? "" : readTicker())
    useEffect(() => {
    ...
    const isCommodity = /^CMD_[A-Z0-9_]+$/.test(ticker)
    const sources = isCommodity
        ? "공개 원자재 스냅샷 · TradingView 선물 연속물"
        : (props.sources || "DART 전자공시 · KRX · 금융위 공공데이터 · 네이버 금융 · SEC EDGAR").trim()
    const held = isCommodity ? false : props.showHeld !== false
    const note = isCommodity
        ? "선물 연속물 기준 · 거래소·서비스별 지연 가능 · 현물·ETF·ETN 수익률은 다를 수 있어요 · 정보 제공 목적"
        : (props.note || "").trim()
