...
const CLOSE_URL = "https://rte5guenhonw9fzn.public.blob.vercel-storage.com/kr_close_latest.json"

interface DivRow { record_date: string; pay_date?: string | null; dps: number }
interface DivSec {
    recent: DivRow[]
    ttm_dps?: number | null
...
const SAMPLE: DivSec = {
    ...
    ttm_dps: 1682, latest_record_date: "2026-06-30", latest_pay_date: "2026-08-28",
    paid_years_10y: 10, stock_kind: "보통주",
...
interface Props { ticker: string; reportUrl: string; closeUrl: string; dark: boolean }
...
export default function PublicDividendHistory(props: Props) {
    ...
    const closeMd = close && close.asOf.length === 8
        ? close.asOf.slice(4, 6) + "/" + close.asOf.slice(6, 8) : ""

    const wrap: CSSProperties = { width: "100%", minHeight: "100%", background: C.bg, fontFamily: FONT, padding: narrow ? "0 12px" : "0 18px", boxSizing: "border-box", color: C.ink }
    const chip: CSSProperties = { fontSize: 10.5, fontWeight: 800, color: C.vt, background: C.vtS, padding: "2px 7px", borderRadius: 6, whiteSpace: "nowrap" }
        ...
        <div ref={rootRef} style={wrap}>
            ...
            <div style={{ background: C.card, borderRadius: 16, padding: narrow ? 14 : 18, boxSizing: "border-box", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 7, marginBottom: 12, flexWrap: "wrap" }}>
                    <span style={{ fontSize: narrow ? 15 : 16, fontWeight: 800, letterSpacing: "-0.3px" }}>배당</span>
                    <span style={{ fontSize: 11.5, color: C.faint, fontWeight: 600 }}>배당기준일 · 사실</span>
                    {sec.stock_kind && sec.stock_kind !== "보통주" ? <span style={chip}>{sec.stock_kind}</span> : null}
                </div>
                ...
                <div style={{ display: "flex", alignItems: "baseline", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
                    {ttm > 0 ? (
                        <>
                            <span style={{ fontFamily: HEAD, fontSize: narrow ? 20 : 23, fontWeight: 800, color: C.vt, letterSpacing: "-0.6px" }}>{won(ttm)}</span>
                            <span style={{ fontSize: 12.5, fontWeight: 700 }}>최근 1년 주당배당금</span>
                        </>
                    ) : (
                        <span style={{ fontSize: 12.5, fontWeight: 700, color: C.sub }}>최근 1년 배당 없음</span>
                    )}
                    {yieldPct != null ? (
                        <span style={{ fontSize: 11.5, color: C.faint, fontWeight: 600 }}>
                            시가배당률 {yieldPct.toFixed(2)}% · {closeMd} 종가 기준
                        </span>
                    ) : null}
                </div>

                <div style={{ fontSize: 11.5, color: C.faint, fontWeight: 600, marginBottom: 12, lineHeight: 1.55 }}>
                    {sec.paid_years_10y ? `최근 10년 중 ${sec.paid_years_10y}년 지급` : "최근 10년 지급 이력 없음"}
                    {sec.upcoming_record_date ? ` · 다음 기준일 ${md(sec.upcoming_record_date)}` : ""}
                </div>

                {/* 이력 */}
                <div style={{ display: "flex", flexDirection: "column" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 8, padding: "0 0 6px", fontSize: 10.5, color: C.faint, fontWeight: 700 }}>
                        <span style={{ flex: 1, minWidth: 0 }}>배당기준일</span>
                        <span style={{ flexShrink: 0, minWidth: narrow ? 62 : 74, textAlign: "right" }}>주당</span>
                        <span style={{ flexShrink: 0, minWidth: narrow ? 54 : 64, textAlign: "right" }}>지급일</span>
                    </div>
                    {rows.map((r, i) => (
                        <div key={i} style={{ display: "flex", alignItems: "center", gap: 8, padding: "9px 0", borderTop: "1px solid " + C.line }}>
                            <span style={{ flex: 1, minWidth: 0, fontSize: 12.5, fontWeight: 700, color: C.ink }}>{md(r.record_date)}</span>
                            <span style={{ flexShrink: 0, minWidth: narrow ? 62 : 74, textAlign: "right", fontSize: 12.5, fontWeight: 800, color: C.vt }}>{won(r.dps)}</span>
                            <span style={{ flexShrink: 0, minWidth: narrow ? 54 : 64, textAlign: "right", fontSize: 11.5, fontWeight: 600, color: C.faint }}>{r.pay_date ? md(r.pay_date) : "미정"}</span>
                        </div>
                    ))}
                </div>
                ...
                <div style={{ fontSize: 10.5, color: C.faint, fontWeight: 500, marginTop: 13, lineHeight: 1.55 }}>
                    {sec.note || "배당기준일 기준 · 기준일 당일 매수로는 받지 못하며 그 전에 보유해야 함 (배당락일 아님)"}
                    <br />
...
addPropertyControls(PublicDividendHistory, {
    ...
    closeUrl: { type: ControlType.String, title: "Close URL", defaultValue: CLOSE_URL },
