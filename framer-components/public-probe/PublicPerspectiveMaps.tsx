...
function catCount(c: any, mkt: string): number {
    const kr = Number(c && c.n_kr) || 0,
        us = Number(c && c.n_us) || 0
    if (mkt === "국장") return kr
    if (mkt === "미장") return us
...
function catLeaders(c: any, mkt: string, sortKey: string): any[] {
    let a = ((c && c.leaders) || []).filter(
        (l: any) =>
            mkt === "전체" || (mkt === "국장" ? l.mkt === "KR" : l.mkt === "US")
    )
    a = a
        .slice()
        .sort((x: any, y: any) =>
...
export default function PublicPerspectiveMaps(props: {
    ...
    if (!data) {
        const sk = (w: any, h: number, r = 10): CSSProperties => ({
            width: w,
            height: h,
            borderRadius: r,
            background: C.track,
            animation: "vpmSh 1.4s ease-in-out infinite",
        })
        return (
            <div style={wrap}>
    ...
    const rows = rawCats
        .map((c) => ({ c, fc: catCount(c, mkt) }))
        .filter((r) => r.fc > 0)
        .sort((a, b) => b.fc - a.fc)
    const maxFc = Math.max(1, ...rows.map((r) => r.fc))
    const selKey =
        sel[lens] && rows.some((r) => r.c.key === sel[lens])
            ? sel[lens]
            : rows[0]
              ? rows[0].c.key
              : ""
    ...
    const seg = (v: string, lb: string) => (
        <button
            ...
            style={{
                ...
                fontSize: 12.5,
                fontWeight: 800,
                background: lens === v ? C.card : "transparent",
                color: lens === v ? C.ink : C.sub,
                boxShadow: lens === v ? "0 1px 3px rgba(0,0,0,0.06)" : "none",
            }}
        >
            {lb}
        </button>
    ...
    const dd = (
        ...
        return (
            ...
            >
                <button
                    onClick={() => setDdOpen(on ? "" : key)}
                    style={{
                        display: "inline-flex",
                        alignItems: "center",
                        gap: 6,
                        border: "none",
                        background: on ? C.violetSoft : C.card,
    ...
    const catIcon = (
        ...
        const Ic = ICON[key]
        return Ic ? (
            <Ic size={size} weight={weight} color={color} style={style} />
        ) : null
    }
    ...
    return (
        <div style={wrap}>
            <div style={{ marginBottom: 12 }}>
                ...
                >
                    같은 시장을 다른 렌즈로 · {lensSub}
                    {meta.generated_at
                        ? " · " + fmtAge(meta.generated_at) + " 갱신"
                        : ""}
                </div>
            </div>
                    ...
                    {selRow ? (
                        ...
                        >
                            ...
                            {shownLeaders.length ? (
                                ...
                                >
                                    {shownLeaders.map((l: any, i: number) => (
                                        <StockCard
                                            key={(l.ticker || "") + i}
                                            l={l}
                                            C={C}
            ...
            >
                분류 = 탐색용 관점(기준 공개) · 집계 = 공시 사실 · 크기 = 종목수
                · 종목은 대표 예시 · 랭킹·점수 아님
            </div>
        </div>
    )
