 ...
 *   (필드 = dart_quarterly_snapshots.jsonl 수집 스키마 정합 → SAMPLE→실데이터 무변환 스왑)
 * 🚨 라이브: 실데이터(≥4분기) 없으면 **자동 숨김**(가짜 추이 노출 금지, RULE 7). 캔버스만 SAMPLE 미리보기.
 * 차트 = 선형(시세 그래프 스타일) + 영역 채움 + 최고/최저점 마커. 막대 아님(2026-06-27 PM).
 * 색 역할: 라인·영역=브랜드 보라(vt) / 개선=초록(데이터신호) / 악화=앰버(주의) — 등락 빨강·파랑과 분리.
 * 테마: Framer 네이티브 추종(body[data-framer-theme]). 반응형 ResizeObserver(px 좌표 = 점 왜곡 없음).
 * 분기 손익(q1/h1/q3)=누적치 가능 → BS기반(부채/유동/ROA) 가장 깨끗, 매출총이익률은 누적 주의(라벨 명시).
 * 배치 = PublicStockReport 재무 요약 *위*(한눈 파악 먼저).
 */
...
// SAMPLE — 턴어라운드 예시(부채↓·수익성↑). 20분기(21Q1~25Q4). 캔버스/데모 전용.
...
export default function PublicQuarterlyTrend(props: Props) {
    ...
    // 차트 좌표 = px (점·라인 왜곡 없음). 카드 내부폭 ≈ root - 패딩.
    const CW = Math.max(80, (w || 360) - (narrow ? 28 : 36))
    const CH = 52
    const PX = 4   // 좌우 여백(점 잘림 방지)
            ...
            <div style={{ background: C.card, borderRadius: 16, padding: narrow ? "15px 14px" : "17px 18px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: 0, marginTop: 10 }}>
                    {METRICS.map((m, mi) => {
                        ...
                        // px 좌표 점 (null 은 건너뛰되 x 위치 보존 = 시간축 정확)
                        const n = raw.length
