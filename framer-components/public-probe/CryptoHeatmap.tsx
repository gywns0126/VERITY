...
const LIGHT = {
    ...
    line: "#e5e8eb", up: "#15c47e", down: "#f04452", accent: "#0ca678", tileInk: "#191f28",
}
const DARK = {
    bg: "#0f1318", card: "#171c23", ink: "#e3e7ec", sub: "#9aa4b1", faint: "#828d9b",
    line: "#252b34", up: "#3ddc97", down: "#ff6b76", accent: "#3ddc97", tileInk: "#ffffff",
}
...
const DEMO = {
    coins: [
        { id: "bitcoin", symbol: "BTC", name: "Bitcoin", market_cap: 1.29e12, current_price: 64663, change_pct: 0.72, change_pct_7d: -3.8, sparkline: [] },
        { id: "ethereum", symbol: "ETH", name: "Ethereum", market_cap: 2.1e11, current_price: 2630, change_pct: 0.67, change_pct_7d: -5.1, sparkline: [] },
        { id: "tether", symbol: "USDT", name: "Tether", market_cap: 1.86e11, current_price: 1, change_pct: 0.0, change_pct_7d: 0.01, sparkline: [] },
        { id: "binancecoin", symbol: "BNB", name: "BNB", market_cap: 8.0e10, current_price: 560, change_pct: 0.8, change_pct_7d: 2.1, sparkline: [] },
        { id: "solana", symbol: "SOL", name: "Solana", market_cap: 6.5e10, current_price: 140, change_pct: -1.2, change_pct_7d: 4.3, sparkline: [] },
        { id: "ripple", symbol: "XRP", name: "XRP", market_cap: 3.1e10, current_price: 0.52, change_pct: 1.9, change_pct_7d: -2.0, sparkline: [] },
    ],
}
...
function boxBg(pct: number | null, up: string, down: string): { bg: string; strong: boolean } {
    ...
    const hex = pct >= 0 ? up : down
    const h = hex.replace("#", "")
    const r = parseInt(h.slice(0, 2), 16), g = parseInt(h.slice(2, 4), 16), b = parseInt(h.slice(4, 6), 16)
    return { bg: `rgba(${r},${g},${b},${a.toFixed(3)})`, strong: a >= 0.45 }
}
...
export default function CryptoHeatmap(props: { dataUrl?: string; dark?: boolean; metric?: Metric }) {
    ...
    const [metric, setMetric] = useState<Metric>(props.metric === "7d" ? "7d" : "24h")
    ...
    return (
        <div ref={rootRef} style={wrap} onMouseLeave={() => setHover(null)}>
            ...
            <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                ...
                <span style={{ ...MONO, fontSize: 11.5, color: C.faint, fontWeight: 700 }}>{coins.length}개 · 박스=시총 · 색={metric} 변동</span>
                ...
                <div style={{ marginLeft: "auto", display: "inline-flex", background: C.line, borderRadius: 9, padding: 2 }}>
                    {(["24h", "7d"] as Metric[]).map((m) => (
                        <button key={m} onClick={() => setMetric(m)} style={{ border: "none", cursor: "pointer", fontFamily: FONT, fontSize: 12, fontWeight: 700, padding: "5px 11px", borderRadius: 7, background: metric === m ? C.card : "transparent", color: metric === m ? C.ink : C.sub }}>{m}</button>
                    ))}
                </div>
            </div>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 3 }}>
                {coins.map((c: any) => {
                    ...
                    const col = boxBg(isFinite(pct) ? pct : null, C.up, C.down)
...
addPropertyControls(CryptoHeatmap, {
    ...
    metric: { type: ControlType.Enum, title: "색 지표", options: ["24h", "7d"], optionTitles: ["24시간", "7일"], defaultValue: "24h" },
})
