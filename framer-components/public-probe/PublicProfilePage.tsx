...
async function fetchProfile(
    ...
    try {
        ...
        return (
            (await get(
                "display_name,email,phone,status,created_at,nickname,avatar,bio,is_admin"
            )) ||
            (await get(
                "display_name,email,phone,status,created_at,nickname,avatar,is_admin"
            )) ||
            (await get(
                "display_name,email,phone,status,created_at,is_admin"
            )) ||
...
function statusMeta(
    ...
    if (status === "approved")
        return { label: "승인 완료", fg: C.green, bg: C.greenSoft }
    if (status === "withdrawn")
        return { label: "탈퇴 처리됨", fg: C.faint, bg: C.line }
    if (status === "pending")
        return { label: "승인 대기", fg: C.blue, bg: C.blueSoft }
...
export default function PublicProfilePage(props: Props) {
    ...
    const saveProfile = async () => {
        ...
        if (v && !NICK_RE.test(v)) {
            setEditMsg(
                "별명은 2~16자, 한글·영문·숫자(._- 허용) · 공백 불가예요"
            )
            return
        }
        const s = loadSession()
        ...
        return (
            <div style={wrap}>
                ...
                >
                    ...
                    >
                        <div
                            style={{ ...skBar(80, 80, 26), marginBottom: 14 }}
                        />
            ...
            <div style={cardStyle}>
                ...
                <div style={{ marginTop: 20 }}>
                    ...
                    <InfoRow
                        C={C}
                        label="가입일"
                        value={fmtDate(profile ? profile.created_at : "")}
                    />
                </div>
                    ...
                    >
                        ...
                        >
                            ...
                            <button
                                ...
                                disabled={busy !== ""}
                                style={btnDangerHalf(C, busy === "withdraw")}
                            >
                                {busy === "withdraw"
                                    ? "처리 중..."
                                    : "탈퇴하기"}
        ...
        >
            ...
            <span
                style={{
                    ...
                    fontSize: 13.5,
                    fontWeight: 600,
                    fontFamily: props.mono ? FONT_MONO : FONT,
