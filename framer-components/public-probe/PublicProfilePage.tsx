 ...
 * 탈퇴: profiles.status = "withdrawn" soft-delete + 세션 파기 + 리다이렉트. 실제 삭제는 관리자.
 * RULE7: 점수/등급/verdict 0. 캔버스에서는 샘플 카드 렌더(부작용 없음).
 *
 * 🚨 2026-07-24 테마 = 자체 내장 CSS 변수(--an-prf-*) 구동. JS 다크 감지 전면 제거 + 헤드 CSS 의존 제거.
 *   <style>{AN_PALETTE} 정적 HTML 정합. 🚨 BustAvatar SVG fill = style 로 넘김(var). 되돌리지 말 것.
 */

const LIGHT = {
    bg: "#f2f4f6", card: "#ffffff", ink: "#191f28", sub: "#4e5968", faint: "#8b95a1",
    line: "#e5e8eb", field: "#f7f8fa", red: "#f04452", redSoft: "#fff0f1", green: "#15c47e",
    greenSoft: "#eafaf3", blue: "#3182f6", blueSoft: "#eef4ff", skBase: "#e7eaee", skHi: "#f3f5f8",
}
const DARK = {
    bg: "#0f1318", card: "#171c23", ink: "#e3e7ec", sub: "#9aa4b1", faint: "#828d9b",
    line: "#252b34", field: "#0f1318", red: "#f04452", redSoft: "#2a1a1d", green: "#34e08a",
    greenSoft: "#0f241c", blue: "#5b9bff", blueSoft: "#152031", skBase: "#232a33", skHi: "#2f3742",
}
const FONT = "'Pretendard', 'Inter', -apple-system, sans-serif"
const FONT_MONO = "'SF Mono', 'JetBrains Mono', 'Menlo', monospace"
const SESSION_KEY = "verity_supabase_session"

// 🎨 팔레트 자체 내장 — LIGHT/DARK 를 CSS 변수(--an-prf-*)로 발행. 정적 HTML 정합. 되돌리지 말 것.
const _ANP = "prf"
const AN_PALETTE =
...
function clearSession() {
    ...
    try {
        sessionStorage.removeItem("verity_session_init")
    } catch (e) {} // 세션 가드 재평가 허용
}
...
async function fetchProfile(
    ...
    try {
        ...
        return (
            (await get("display_name,email,phone,status,created_at,nickname,avatar,bio,is_admin")) ||
...
function BustAvatar(props: { size: number; color: string }) {
    ...
    return (
        <svg width={size} height={size} viewBox="0 0 48 48" fill="none" aria-hidden="true">
            <circle cx="24" cy="17" r="8.5" style={{ fill: color }} />
            <path
...
export default function PublicProfilePage(props: Props) {
    ...
    useEffect(() => {
        ...
        // 🚨 phase="member" 를 fetchProfile 완료까지 미룸 — 로딩 스켈레톤이 실제 느린 구간 동안 보이게(2026-07-18). 안전 타임아웃(6s).
        let alive = true
        const settle = () => {
            if (alive) setPhase("member")
        }
    ...
    const saveProfile = async () => {
        ...
        if (v && !NICK_RE.test(v)) {
            setEditMsg("별명은 2~16자, 한글·영문·숫자(._- 허용) · 공백 불가예요")
            return
        }
        const s = loadSession()
        ...
        if (r.ok) {
            ...
            setEditMsg(
                r.status === 409 ? "이미 사용 중인 별명이에요" : "저장에 실패했어요 — 잠시 후 다시 시도해 주세요"
            )
        }
    }
    ...
    const pickAvatar = () => {
        if (!avatarBusy && fileRef.current) fileRef.current.click()
    }
    ...
    const wrap: React.CSSProperties = {
        width: "100%", minHeight: "100vh", background: C.bg, fontFamily: FONT, color: C.ink,
        display: "flex", alignItems: "flex-start", justifyContent: "center",
        padding: "40px 20px 64px", boxSizing: "border-box",
    }
    const cardStyle: React.CSSProperties = {
        width: "100%", maxWidth: 460, background: C.card, borderRadius: 22,
        boxShadow: "0 2px 16px rgba(0,0,0,0.06)", padding: 28, boxSizing: "border-box",
    }

    if (phase === "loading") {
        // 스켈레톤 — 최초 로딩 동안 카드 형태를 본떠 표시. 관리자든 아니든 로딩 phase 공통.
        ...
        const skBar = (w: number | string, h: number, r: number = 8, mt: number = 0): React.CSSProperties => ({
            ...
            backgroundSize: "800px 100%", animation: "ppShimmer 1.4s ease-in-out infinite",
        })
        return (
            <div style={wrap}>
            ...
            <div style={cardStyle}>
                ...
                {!editMode ? (
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center", textAlign: "center" }}>
                        <div style={{ width: 80, height: 80, borderRadius: 26, background: C.field, display: "flex", alignItems: "center", justifyContent: "center", marginBottom: 14, overflow: "hidden" }}>
                            {profile && profile.avatar ? (
                                <img src={profile.avatar} alt="" width={80} height={80} style={{ width: 80, height: 80, objectFit: "cover", display: "block" }} />
                            ) : (
                        ...
                        <button
                            type="button"
                            onClick={() => {
                                setNickDraft((profile && profile.nickname) || "")
                                setBioDraft((profile && profile.bio) || "")
                                setEditMode(true)
                                setEditMsg("")
                            }}
                    ...
                    <div style={{ display: "flex", flexDirection: "column", alignItems: "center" }}>
                        ...
                        >
                            {profile && profile.avatar ? (
                                <img src={profile.avatar} alt="" width={80} height={80} style={{ width: 80, height: 80, borderRadius: 26, objectFit: "cover", display: "block" }} />
                            ) : (
                                <BustAvatar size={50} color={C.sub} />
                            )}
                        </button>
                        ...
                        <div style={{ display: "flex", gap: 4, justifyContent: "center", marginTop: 14 }}>
                            <button
                                ...
                                style={{ border: "none", background: "transparent", cursor: "pointer", padding: "7px 14px", borderRadius: 999, fontSize: 12.5, fontWeight: 700, fontFamily: FONT, color: C.faint }}
                            >
                                취소
                            </button>
                            <button
                                type="button"
                                onClick={saveProfile}
                                style={{ border: "none", background: C.ink, color: C.card, cursor: "pointer", padding: "7px 18px", borderRadius: 999, fontSize: 12.5, fontWeight: 800, fontFamily: FONT }}
                            >
                                저장
                            </button>
                        </div>
                ...
                <div style={{ marginTop: 20 }}>
                    {profile && profile.phone ? (
                        <InfoRow C={C} label="전화번호" value={profile.phone} mono />
                    ) : null}
                ...
                <a href="/policy" style={{ display: "block", textAlign: "center", marginTop: 18, color: C.faint, fontSize: 12, fontWeight: 600, textDecoration: "none" }}>
                    이용약관 · 개인정보처리방침
                </a>
            </div>
        </div>
...
function InfoRow(props: { C: any; label: string; value: string; mono?: boolean }) {
    const C = props.C
    return (
...
function btnSolid(C: any, loading: boolean): React.CSSProperties {
    return {
...
function btnGhostDanger(C: any): React.CSSProperties {
    return {
        width: "100%", padding: "12px 0", border: "none", borderRadius: 14, background: "transparent",
        color: C.red, fontSize: 13.5, fontWeight: 700, fontFamily: FONT, cursor: "pointer",
    }
}

function btnFlatHalf(C: any): React.CSSProperties {
    return {
        flex: 1, padding: "11px 0", border: "none", borderRadius: 12, background: C.field,
        color: C.sub, fontSize: 13.5, fontWeight: 700, fontFamily: FONT, cursor: "pointer",
    }
}

function btnDangerHalf(C: any, loading: boolean): React.CSSProperties {
    return {
        flex: 1, padding: "11px 0", border: "none", borderRadius: 12, background: C.red,
        color: "#ffffff", fontSize: 13.5, fontWeight: 800, fontFamily: FONT,
        cursor: loading ? "wait" : "pointer", opacity: loading ? 0.6 : 1,
    }
}
