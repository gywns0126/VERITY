                ...
                <div style={{ background: C.card, borderRadius: 12, padding: "10px 13px", marginBottom: 10, boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                    ...
                    {myBelow5.slice(0, 8).map((r: any, i: number) => (
                        <div key={i} style={{ display: "flex", alignItems: "baseline", gap: 8, padding: "4px 0", fontSize: 12.5 }}>
                            ...
                            <span style={{ fontSize: 9.5, fontWeight: 800, color: myHold.has(String(r.ticker)) ? (C.green || "#0ca678") : C.blue, background: myHold.has(String(r.ticker)) ? (C.greenSoft || "#eafaf3") : (C.blueSoft || "#eef4ff"), borderRadius: 5, padding: "1.5px 6px" }}>{myHold.has(String(r.ticker)) ? "내 보유" : "관심"}</span>
                            <span style={{ marginLeft: "auto", fontWeight: 800, color: C.ink, fontVariantNumeric: "tabular-nums" }}>{r.pct != null ? Number(r.pct).toFixed(2) + "%" : "—"}</span>
                        </div>
                    ))}
