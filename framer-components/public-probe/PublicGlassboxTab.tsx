 ...
 * 🚨 게이트 도달 프레이밍(2026-07-04): N_eff ≥ 252 도달 시 "진행률 99.9%" 대신 다음 관문(N≥684 DSR)으로 재타겟.
 * RULE 6 — ⓘ/문구 평문 사전 작성. 런타임 LLM 0. ⓘ = 항상 표시, PC hover / 모바일 탭, 툴팁 clamp.
 * 스켈레톤(2026-07-05 PM): 실사이트 로딩 = 토스식 shimmer 스켈레톤. SAMPLE 은 캔버스 전용.
 *
 * 🚨 2026-07-24 테마 = 자체 내장 CSS 변수(--an-gbx-*) 구동. JS 다크 감지 전면 제거 + 헤드 CSS 의존 제거.
 *   <style>{AN_PALETTE} 정적 HTML 정합. 되돌리지 말 것.
 */

const LIGHT = { bg: "#f2f4f6", card: "#ffffff", ink: "#191f28", sub: "#4e5968", faint: "#8b95a1", line: "#e5e8eb", vt: "#6c5ce7", vtS: "#f0edff", vg: "#0ca678", vgS: "#e7faf0", amber: "#ff9500", amberS: "#fff6e9", tipBg: "#191f28", tipFg: "#ffffff", skBase: "#e9edf1", skHi: "#f3f5f7" }
const DARK = { bg: "#0f1318", card: "#171c23", ink: "#e3e7ec", sub: "#9aa4b1", faint: "#828d9b", line: "#252b34", vt: "#a99bff", vtS: "#241f3a", vg: "#7fffa0", vgS: "#11281d", amber: "#ff9500", amberS: "#2a2113", tipBg: "#222a33", tipFg: "#e3e7ec", skBase: "#222a33", skHi: "#2d3742" }
const FONT = "Pretendard, -apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', sans-serif"

// 🎨 팔레트 자체 내장 — LIGHT/DARK 를 CSS 변수(--an-gbx-*)로 발행. 정적 HTML 정합. 되돌리지 말 것.
const _ANP = "gbx"
const AN_PALETTE =
...
const SAMPLE = {
    ...
    gate: { target_n: 252, milestone: "N=252 IC 게이트 (Bailey-López de Prado, 2027-05 목표)", best_signal_n: 145.2, progress_pct: 57.6 },
    signals: [
        { signal: "brain_production", status: "채점 진행 중", n: 726, n_eff: 145.2, label: "표본 N≥100 누적 (잠정 — 유의성 미검증)", gate_status: "가설 (게이트 N≥252 미도달, 진척 57.6%)", source: "prediction_ic_history.jsonl (Brain 종합 verdict, prediction_scoring v0)" },
        { signal: "xgb_ml", status: "trail 누적, 채점 도달 0", n: null, n_eff: null, label: "데이터 없음", gate_status: "가설 (관측 0)", source: "ml_prediction_ic_history.jsonl (XGB up_probability shadow)" },
        { signal: "factor", status: "IC 시계열 누적 중", n: 53, n_eff: null, label: "예비 (N<100, 검증 진행 중)", gate_status: "가설 (게이트 N≥252 미도달, 진척 21.0%)", source: "factor_ic_history.json (ic_stats machinery, forward_days 30)" },
        { signal: "sector", status: "채점 DEFERRED (return source 미확정)", n: 0, n_eff: null, label: "데이터 없음 (채점 보류)", gate_status: "가설 (채점 보류 — return source 미검증)", source: "prediction_trail.jsonl (target_type=sector)" },
    ] as Sig[],
}
...
export default function PublicGlassboxTab(props: Props) {
    ...
    // 표본 게이트 도달 = 판정 "시작선" 도달일 뿐, 유의성 통과(검증 완료) 아님.
        ...
        return (
            <span style={{ position: "relative", display: "inline-block" }}>
                <span
                    ...
                    {...hov}
                    style={{ display: "inline-flex", alignItems: "center", justifyContent: "center", width: "1.42em", height: "1.42em", marginLeft: "0.35em", borderRadius: "50%", background: "#6c5ce7", color: "#fff", fontSize: "0.62em", fontWeight: 700, lineHeight: 1, verticalAlign: "middle", position: "relative", top: "-0.08em", cursor: "help" }}
                >
                    i
                </span>
                {isOpen && (
                    <span
                        ...
                        style={{ position: "absolute", top: "calc(100% + 4px)", left: tipBox.left, zIndex: 50, display: "block", width: tipBox.width, background: C.tipBg, color: C.tipFg, borderRadius: 12, padding: "11px 13px", fontSize: 12.5, fontWeight: 500, lineHeight: 1.55, boxShadow: "0 6px 20px rgba(0,0,0,0.18)", whiteSpace: "normal", textAlign: "left" }}
                    >
    ...
    const wrap: CSSProperties = { width: "100%", height: "100%", maxHeight: "100%", overflowY: "auto", overflowX: "hidden", background: C.bg, fontFamily: FONT, padding: pad, boxSizing: "border-box", color: C.ink }
    ...
    // 로딩 스켈레톤 — 실데이터 도착 전 가짜 숫자 노출 0 (토스식 shimmer)
    if (!onCanvas && !data) {
        const base = C.skBase
        const hi = C.skHi
        const sk = (wd: number | string, h: number, r = 8, mt = 0): CSSProperties => ({ width: wd, height: h, borderRadius: r, marginTop: mt, background: base, backgroundImage: `linear-gradient(90deg, ${base} 25%, ${hi} 37%, ${base} 63%)`, backgroundSize: "800px 100%", animation: "vgbShimmer 1.4s ease-in-out infinite" })
        ...
        <div ref={rootRef} style={wrap}>
            ...
            <div style={{ ...card, padding: "16px 16px", marginTop: 12 }}>
                ...
                <div style={{ fontSize: 11.5, color: C.faint, fontWeight: 600, lineHeight: 1.5 }}>
                    {gateReached ? (nextMs ? `IC 게이트(N≥${targetN}) 도달 ✓ · 다음 ${nextMs.label}(N≥${nextMs.n})까지 ${f2(nextPct, 0)}% · 유의성 판정은 표본 쌓이며 진행 · 검증 완료 아님` : `전 표본 관문(N≥684) 통과 · 유의성·DSR 최종 판정 단계 · 검증 완료 아님`) : `유효표본이 통계 유의 최소선(N≥${targetN})에 닿은 정도 ${f2(progress, 1)}% · 관측 횟수 기반(시간 진척 아님)`}
                </div>
            </div>
                        ...
                        <div key={ms.n} style={{ display: "flex", gap: 11, alignItems: "flex-start", padding: "8px 0", borderTop: i === 0 ? "none" : `1px solid ${C.line}` }}>
                            ...
                            <div style={{ flex: 1, minWidth: 0 }}>
                                <div style={{ fontSize: 12.5, fontWeight: 800, color: reached ? C.ink : C.faint }}>
                                    N≥{ms.n} · {ms.label}
                                    {here ? <span style={{ color: C.vt, fontWeight: 800 }}>{" "}← 현재 N_eff {f2(curN, 0)}</span> : null}
                                </div>
                    ...
                    return (
                        <div key={i} style={{ ...card, padding: "13px 16px" }}>
                            <div style={{ display: "flex", alignItems: "center", gap: 8, flexWrap: "wrap", marginBottom: 4 }}>
                                ...
                                <span style={{ fontSize: 10.5, fontWeight: 800, color: hasN ? C.vg : C.faint, background: hasN ? C.vgS : C.line, borderRadius: 6, padding: "2px 8px" }}>{s.label || "데이터 없음"}</span>
                            </div>
                            <div style={{ display: "flex", gap: 8, alignItems: "baseline", flexWrap: "wrap" }}>
                                <span style={{ fontSize: 13.5, fontWeight: 800, display: "inline-flex", alignItems: "center" }}>
                                    {s.n_eff != null ? `관측 ${f2(s.n, 0)}건 · 유효표본 ${f2(s.n_eff, 0)}` : hasN ? `관측 ${f2(s.n, 0)}건` : "관측 0"}
                                    <Info k="표본 N" uid={"sig" + i} />
                                </span>
                                <span style={{ fontSize: 11.5, color: C.faint, fontWeight: 600 }}>{s.gate_status || "게이트 미도달"}</span>
                            </div>
                            {srcShort && (
                                <div style={{ fontSize: 10.5, color: C.faint, fontWeight: 600, marginTop: 8, paddingTop: 8, borderTop: `1px solid ${C.line}` }}>출처 · {srcShort}</div>
                            )}
                        </div>
                    )
                })}
            ...
            <div style={{ background: C.amberS, borderRadius: 14, padding: "13px 15px" }}>
                <div style={{ fontSize: 12.5, fontWeight: 800, color: C.amber, marginBottom: 4 }}>점수·등급·적중률·IC·기댓값 = 검증 전까지 비공개</div>
