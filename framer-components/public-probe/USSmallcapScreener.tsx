...
export default function USSmallcapScreener(props: { width?: number; dark?: boolean; reportPath?: string; initialFilter?: string }) {
  ...
  if (!data) {
    ...
    const bar = (w: any, h: number, r = 8) => ({
      ...
      backgroundSize: "800px 100%", animation: "vusShimmer 1.4s ease-in-out infinite",
    })
    return (
      <div style={shell}>
        <style>{"@keyframes vusShimmer{0%{background-position:-400px 0}100%{background-position:400px 0}}"}</style>
