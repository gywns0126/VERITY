 ...
 * 데이터 = /api/stock_news?code=종목코드 (네이버 금융 종목뉴스 라이브). RULE 6: LLM 해설 0. RULE 7: 사실만.
 * 종목 = prop ticker → URL ?q → verity_last_ticker. in-page 전환 추종 1s 폴링. 데이터 없으면 graceful 숨김.
 *
 * 🚨 2026-07-24 테마 = 자체 내장 CSS 변수(--an-snw-*) 구동. JS 다크 감지 전면 제거 + 헤드 CSS 의존 제거.
...
const COMMODITY_NAMES: Record<string, string> = {
    CMD_GOLD: "금", CMD_SILVER: "은", CMD_COPPER: "구리", CMD_WTI: "WTI 원유",
    CMD_BRENT: "브렌트유", CMD_NATGAS: "천연가스", CMD_CORN: "옥수수", CMD_WHEAT: "밀",
    CMD_SOYBEAN: "대두", CMD_COFFEE: "커피", CMD_SUGAR: "설탕", CMD_COTTON: "면화",
}
...
// 🎨 팔레트 자체 내장 — LIGHT/DARK 를 CSS 변수(--an-snw-*)로 발행. 정적 HTML 정합. 되돌리지 말 것.
const _ANP = "snw"
const AN_PALETTE =
...
export default function PublicStockNews(props: Props) {
    ...
    /* 뉴스 로드 — 국내 종목코드 또는 직접 원자재 CMD_* 변경 시 */
    useEffect(() => {
        if (onCanvas) return
        const code = String(tk).trim()
        if (!/^\d{6}$/.test(code) && !COMMODITY_NAMES[code]) { setItems([]); return }
        let alive = true
        setLoading(true)
    ...
    const commodityName = COMMODITY_NAMES[tk] || ""
        ...
        <div ref={rootRef} style={wrap}>
            ...
            <div style={{ background: C.card, borderRadius: 16, padding: narrow ? 14 : 18, boxSizing: "border-box", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 7, marginBottom: 12, flexWrap: "wrap" }}>
                    <span style={{ fontSize: narrow ? 15 : 16, fontWeight: 800, letterSpacing: "-0.3px" }}>{commodityName ? `원자재 뉴스 · ${commodityName}` : "종목 뉴스"}</span>
                ...
                <div style={{ fontSize: 10.5, color: C.faint, fontWeight: 500, marginTop: 13, lineHeight: 1.55 }}>
                    {commodityName
                        ? "네이버 검색 API · Google News RSS · 매체·시각·출처는 사실 · 선물·현물·ETF 가격은 서로 다를 수 있어요"
