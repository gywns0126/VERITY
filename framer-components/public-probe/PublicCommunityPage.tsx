...
const DEMO_FEED = [
    {
        ...
        avatar: "",
        stance: "bull",
        note: "수주 잔고 증가 + 부채비율 하향 추세. 다음 분기 마진 확인 후 재검토.",
    ...
    },
    {
        ...
        avatar: "",
        stance: "watch",
        note: "밸류는 부담스러운데 수요가 계속 확인됨. 조정 오면 다시 본다.",
    ...
    },
    {
        ...
        avatar: "",
        stance: "bull",
        note: "HBM 증설 스케줄 그대로면 하반기 실적 방향은 위라고 본다.\n리스크 = 환율.",
    ...
    },
    {
        ...
        avatar: "",
        stance: "bear",
        note: "신사업 비용이 아직 무겁다. 흑자 전환 확인 전까진 보수적으로.",
...
export default function PublicCommunityPage(props: Props) {
    ...
    useEffect(() => {
        ...
        fetch(base + "/api/thesis_feed?limit=" + cap, {
            headers: h,
            cache: "no-store",
        })
            .then((r) => (r.ok ? r.json() : null))
    ...
    const goStock = (tk: string) => {
        ...
        const p = (
            kr ? stockPath || "/stock" : usStockPath || "/us/stock"
        ).replace(/\/+$/, "")
    ...
    const matches = useMemo(() => {
        ...
        for (const tk in names) {
            ...
            if (
                tk.toLowerCase().indexOf(key) >= 0 ||
                nm.toLowerCase().indexOf(key) >= 0
            )
                out.push([tk, nm])
        ...
        const rk = (e: [string, string]) =>
            e[0].toLowerCase() === key
                ? 0
                : e[0].toLowerCase().indexOf(key) === 0
                  ? 1
                  : e[1].toLowerCase().indexOf(key) === 0
                    ? 2
                    : 3
    ...
    const shown = useMemo(() => {
        let arr = filterTk
            ? feed.filter((it) => it.ticker === filterTk)
            : feed.slice()
        if (sort === "hot")
            arr = arr
                .slice()
            ...
            f.map((x) =>
                x.id === it.id
                    ? {
                          ...x,
                          liked,
                          likes: Math.max(0, x.likes + (liked ? 1 : -1)),
                      }
                    : x
            )
        )
    ...
    const sk = (sw: any, sh: number, r = 6): CSSProperties => ({
        ...
        animation: "vcpShimmer 1.4s ease-in-out infinite",
        flexShrink: 0,
    })
    ...
    return (
        <div style={wrap}>
            ...
            <div style={col}>
                ...
                >
                    종목 관점을 나누는 공간 · 모든 글은 이용자 개인 의견이며
                    AlphaNest 의 분석·판단·추천이 아니에요
                </div>
                        ...
                        {(
                            ...
                            <button
                                ...
                                style={{
                                    ...
                                    fontWeight: 800,
                                    background:
                                        sort === k ? C.card : "transparent",
                                    color: sort === k ? C.ink : C.faint,
                                    boxShadow:
                                        sort === k
                    ...
                    >
                        <button
                            ...
                            style={{
                                ...
                                fontSize: 12,
                                fontWeight: 700,
                                background: !filterTk ? C.vg : C.card,
                                color: !filterTk ? C.onAccent : C.sub,
                            }}
                        >
                            전체
                        </button>
                        {(filterTk && tickers.indexOf(filterTk) < 0
                            ? [filterTk, ...tickers]
                            : tickers
                        ).map((tk) => (
                            <button
                                ...
                                style={{
                                    ...
                                    fontSize: 12,
                                    fontWeight: 700,
                                    background: filterTk === tk ? C.vg : C.card,
                                    color: filterTk === tk ? C.onAccent : C.sub,
                                    whiteSpace: "nowrap",
                                }}
                            >
                                {tkName(tk)}
                            ...
                            <div key={it.id} style={card}>
                                ...
                                {it.ticker && (
                                    ...
                                    >
                                        ...
                                        <span style={stanceStyle(it.stance)}>
                                            {STANCE_LABEL[it.stance] || "관망"}
                                        </span>
                                    </div>
                                )}
                                ...
                                >
                                    <button
                                        ...
                                        style={{
                                            ...
                                            fontSize: 12,
                                            fontWeight: 700,
                                            color: it.liked ? C.up : C.faint,
                                        }}
                                    >
                                        <Heart
                                            size={17}
                                            weight={
                                                it.liked ? "fill" : "regular"
                                            }
                                            color={it.liked ? C.up : C.faint}
                                        />
                ...
                >
                    피드의 모든 글 = 이용자 개인 의견 · AlphaNest 의
                    분석·판단·추천 아님 · 부적절한 글은 ⋯ 메뉴로 신고
                </div>
            </div>
        </div>
    )
