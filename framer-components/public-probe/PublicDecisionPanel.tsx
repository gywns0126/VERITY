...
const DEMO = {
    ...
    peer: {
        sector: "건설",
        rows: [
            { key: "PER", value: "8.3", median: "18.6", vs: "below" },
            { key: "PBR", value: "0.6", median: "1.4", vs: "below" },
            { key: "ROE", value: "9.7%", median: "7%", vs: "above" },
            { key: "부채비율", value: "84%", median: "87%", vs: "below" },
            { key: "영업이익률", value: "5.2%", median: "3.2%", vs: "above" },
        ],
    },
...
export default function PublicDecisionPanel(props: Props) {
    ...
    const model = useMemo(() => {
        ...
        const dil = (() => {
            ...
            let n = 0
            for (const k of DILUTIVE) n += Number(c[k]) || 0
            return n
        })()
    ...
    const searchBar = (
        <div style={{ position: "relative", marginBottom: 18 }}>
            ...
            <input
                style={inputStyle}
                placeholder={`종목 검색 (이름·코드)${universe.length ? ` · 전 종목 ${universe.length}개` : ""}`}
                value={query}
    ...
    const m = model
    const noSignal =
        (m.insNet == null || m.insNet === 0) &&
        (m.foreignNet == null || m.foreignNet === 0) &&
