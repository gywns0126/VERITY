 ...
 *
 * 🚨 2026-07-24 테마 = 자체 내장 CSS 변수(--an-mbf-*) 구동. JS 다크 감지 전면 제거 + 헤드 CSS 의존 제거.
 *   <style>{AN_PALETTE} 정적 HTML 정합. 되돌리지 말 것.
 */

const LIGHT = {
    bg: "#f2f4f6", card: "#ffffff", ink: "#191f28", sub: "#4e5968", faint: "#8b95a1",
    line: "#e5e8eb", up: "#f04452", down: "#3182f6", vg: "#6c5ce7", vgS: "#f0edff", warn: "#ff9500", onAccent: "#ffffff",
}
const DARK = {
    bg: "#10141a", card: "#171c23", ink: "#e3e7ec", sub: "#9aa4b1", faint: "#828d9b",
    line: "#252b34", up: "#f04452", down: "#5b9bff", vg: "#a99bff", vgS: "#241f3a", warn: "#ffb340", onAccent: "#0f1318",
}
...
// 🎨 팔레트 자체 내장 — LIGHT/DARK 를 CSS 변수(--an-mbf-*)로 발행. 정적 HTML 정합. 되돌리지 말 것.
const _ANP = "mbf"
const AN_PALETTE =
    ...
    sections: [
        {
            ...
            recap: { date: "07/10", kospi: 0.62, kosdaq: 1.15, kospi_close: 7291.91, kosdaq_close: 794.0, headline: "코스피는 올랐지만 종목 2,633개 중 1,587개는 내렸어요" },
            items: [
        ...
        },
        {
            ...
            items: [{ ticker: "CNXC", name: "Concentrix", text: "10-K/Q 재무 공시 제출 → 재무 반영 완료" }],
        },
        {
            title: "최근 7일 내부자 변동",
            note: "DART 보고 사실 · 증감 주식수",
            items: [{ ticker: "402340", name: "SK스퀘어", text: "12,111,300주 매수 (07-01)" }],
        },
    ],
...
function getToken(): string {
    ...
    try {
        ...
        // 🚨 만료 토큰 = 미로그인 취급 (2026-07-14). HoldingsTab getToken 과 동기.
...
export default function PublicMorningBriefing(props: Props) {
    ...
    const loadHoldings = useCallback(() => {
        ...
        // 🚨 토큰 있으면 즉시 로그인 상태 확정 (isDemo=false) — API 실패해도 "로그인하면…" CTA 뜨는 사고 방지(2026-07-14).
        setIsDemo(false)
        setRows([])
        setLoading(true)
        fetch(base + "/api/holdings", { headers: { Authorization: "Bearer " + token } })
            ...
            .then((d) => {
                setRows(Array.isArray(d) ? d : d && Array.isArray(d.holdings) ? d.holdings : [])
            })
            .catch(() => setRows([]))
            .finally(() => setLoading(false))
    }, [base, onCanvas])
    // 마운트 + 로그인/로그아웃(verity_auth_change · 다른 탭 storage) 재평가 (HoldingsTab 동기, 2026-07-14).
    useEffect(() => {
        loadHoldings()
    ...
    // 시장 브리핑 로드 — sessionStorage cache-fallback
    useEffect(() => {
        if (onCanvas) return
        let alive = true
        const fallback = () => {
        ...
        fetch(briefUrl || BRIEF_URL, { cache: "no-store" })
            ...
            .then((d) => {
                ...
                if (d && Array.isArray(d.sections)) {
                    setBrief(d)
                    try {
                        sessionStorage.setItem("daily_briefing", JSON.stringify(d))
                    } catch (e) {
                        /* ignore */
                    }
                } else fallback()
    ...
    // embargo 해제 타이머 — publish_at 시각에 재렌더.
    useEffect(() => {
    ...
    const fmtLevel = (v: any) =>
        typeof v === "number" && isFinite(v) ? v.toLocaleString("en-US", { minimumFractionDigits: 2, maximumFractionDigits: 2 }) : ""
    ...
    // 발행 시각 = JSON publish_at 우선, 없으면 generated_at 폴백.
    const pubHM = (iso: any) => {
        const m = String(iso || "").match(/T(\d{2}:\d{2})/)
        return m ? m[1] : ""
    }
    const pubTime = brief ? pubHM(brief.publish_at) || pubHM(brief.generated_at) : ""
    // embargo — publish_at 전에는 ② 시장 브리핑 미노출, "발행 예정" 표시.
    ...
    // 카드 밖 제호 + 형제 카드 2장 (개인 / 시장).
    const shell: CSSProperties = { fontFamily: FONT, width: "100%", boxSizing: "border-box", color: C.ink, display: "flex", flexDirection: "column", gap: 12 }
    const card: CSSProperties = { background: C.card, borderRadius: 18, padding: narrow ? "16px 16px" : "20px 20px", boxSizing: "border-box" }
    const cta: CSSProperties = { ...card, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10, padding: narrow ? "15px 16px" : "16px 18px", cursor: "pointer" }
    // 명도 위계 — 라벨은 회색 캡션으로 물러나고, 검정은 콘텐츠에만.
    ...
    return (
        <div ref={rootRef} style={shell}>
            ...
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8, padding: "0 4px" }}>
                <span style={{ fontSize: 18, fontWeight: 800, letterSpacing: "-0.4px" }}>모닝 브리핑</span>
                ...
                <div style={{ ...card, paddingBottom: 0 }}>
                    <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8 }}>
                        ...
                        <span style={{ fontSize: 10.5, color: C.faint, fontWeight: 600 }}>
                            {"평단 입력 기준 · " + (closeDate ? closeDate.slice(5).replace("-", "/") + " 종가 기준" : "전일 종가 대비")}
                        </span>
                    </div>
                    <div style={{ fontSize: narrow ? 23 : 26, fontWeight: 800, letterSpacing: "-1px", margin: "3px 0 2px", fontVariantNumeric: "tabular-nums" }}>
                    ...
                    {asset.dayPct != null ? (
                        <div style={{ fontSize: 13.5, fontWeight: 800, color: upC(asset.dayChange), fontVariantNumeric: "tabular-nums" }}>
                            ...
                            {(asset.dayChange >= 0 ? "+" : "") + wonCompact(asset.dayChange)}{" "}
                            ({(asset.dayPct >= 0 ? "+" : "") + asset.dayPct.toFixed(2)}%)
                            {asset.hasUncovered && (
                                <span style={{ fontSize: 10.5, fontWeight: 600, color: C.faint, marginLeft: 6 }}>국내 종목 기준</span>
                            )}
                        </div>
                    ) : (
                        <div style={{ fontSize: 12.5, fontWeight: 700, color: C.faint }}>종가 데이터 대기</div>
                    )}
                    {/* 움직인 종목 — 컴팩트 행 */}
                            ...
                            {asset.movers.map((m: any) => (
                                ...
                                >
                                    ...
                                    <div style={{ display: "flex", alignItems: "center", gap: 9, flexShrink: 0, fontVariantNumeric: "tabular-nums" }}>
                                        {m._dayPct != null && (
                                            <span style={{ fontSize: 12.5, fontWeight: 800, color: upC(m._day) }}>
                                                {(m._dayPct >= 0 ? "+" : "") + m._dayPct.toFixed(1)}%
                                            </span>
                                        )}
                                        <span style={{ fontSize: 12, fontWeight: 700, color: upC(m._day), minWidth: 64, textAlign: "right" }}>
                                            {(m._day >= 0 ? "+" : "") + wonCompact(m._day)}
                                        </span>
                                    </div>
                                </div>
                            ))}
            ...
            <div style={card}>
                {!brief ? (
                    ...
                    <div style={{ fontSize: 12.5, color: C.faint, fontWeight: 600, lineHeight: 1.6 }}>
                        오늘 시장 브리핑은{" "}
                        <span style={{ color: C.ink, fontWeight: 800 }}>{pubTime || "07:30"}</span>{" "}
                        에 발행돼요
                    </div>
                ) : (
                    <div>
                        {/* 1면 배너 — 지수 레벨 + 큰 등락% + 흐름 한 줄 */}
                        {banner && (
                            <div>
                                <div style={{ display: "flex", gap: narrow ? 20 : 28, alignItems: "flex-end", flexWrap: "wrap" }}>
                                    {[
                                        ["코스피", banner.kospi, banner.kospi_close],
                                        ["코스닥", banner.kosdaq, banner.kosdaq_close],
                                    ].map(([lb, pct, lv]: any) => (
                                        <div key={lb}>
                                            <div style={{ display: "flex", alignItems: "baseline", gap: 6 }}>
                                                <span style={idxLabel}>{lb}</span>
                                                {fmtLevel(lv) && (
                                                    <span style={{ fontSize: 11.5, fontWeight: 600, color: C.faint, fontVariantNumeric: "tabular-nums" }}>{fmtLevel(lv)}</span>
                                                )}
                                            </div>
                                            <div style={{ marginTop: 2, fontSize: narrow ? 22 : 25, fontWeight: 800, letterSpacing: "-0.7px", color: pctColor(pct), fontVariantNumeric: "tabular-nums", lineHeight: 1.1 }}>
                                                {fmtPct(pct)}
                                            </div>
                                        </div>
                                    ))}
                                    <div style={{ marginLeft: "auto", alignSelf: "flex-start", fontSize: 10.5, fontWeight: 600, color: C.faint, whiteSpace: "nowrap" }}>
                                        {banner.date} 종가
                                        {Number(brief.warnings_n) > 0 ? " · 시장경보 " + brief.warnings_n : ""}
                                    </div>
                                </div>
                                {banner.headline && (
                                    <div style={{ marginTop: 9, fontSize: narrow ? 14 : 15, fontWeight: 800, letterSpacing: "-0.2px", color: C.ink, lineHeight: 1.45 }}>
                                        {banner.headline}
                                    </div>
                                )}
                            </div>
                        ...
                        {secs.map((s: any, si: number) => {
                            ...
                            return (
                                <div key={si} style={{ marginTop: divided ? 16 : 0, paddingTop: divided ? 16 : 0, borderTop: divided ? `1px solid ${C.line}` : "none" }}>
                                        ...
                                        {items.map((it: any, i: number) => (
                                            <div key={i}>
                                                ...
                                                <div style={{ display: "flex", gap: 8, alignItems: "baseline", fontSize: narrow ? 12.5 : 13, lineHeight: 1.5 }}>
                                                    {/* 종목명 = ink 700 + 흐린 밑줄. 보라는 액션 전용 */}
                                                    <span
                                                        onClick={() => goStockTk(String(it.ticker || ""))}
                                                        style={{ flexShrink: 0, fontWeight: 700, color: it.ticker ? C.ink : C.faint, cursor: it.ticker ? "pointer" : "default", textDecoration: it.ticker ? "underline" : "none", textDecorationColor: C.line, textUnderlineOffset: 3 }}
                                                    >
                                                        {it.name || it.ticker}
                                                    </span>
                                                    {it.mover && it.text ? (
                                                        moverText(String(it.text))
                                                    ) : (
                                    ...
                                    {extra > 0 && (
                                        <button
                                            onClick={() => setOpenSec((o) => ({ ...o, [s.title]: !open }))}
