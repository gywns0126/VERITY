 ...
 *
 * 기존 인프라 100% 재사용 (신규 비용 0):
 *   - 실시간 = Railway KIS WebSocket → SSE /stream/{ticker} (snapshot/candles/candle/trade)
 *   - 장 마감/SSE 공백 시 fallback = 공개 recommendations.json 의 sparkline (number[])
 *   - 차트 = 순수 SVG (라이브러리 0), StockDetailPanel CandleChart 패턴
 *
 * 🚨 사실(시세)만. 점수·등급·추천·매매신호 없음 (RULE 7). 실 KR 6자리 ticker 만 스트리밍.
 *    Toss Open API 도입 원복(2026-06-19) — 실시간은 기존 KIS WS 가 우위라 그대로 소비.
 */
...
// 실 KR ticker 만 (스트리밍 가능 종목). 가짜/해외 직접 ticker 제외.
...
// ── 라인(sparkline) 폴백 — 장 마감 시 recommendations.json 종가 시계열 ──
...
// ── 공개 recommendations.json 1회 로드 (장 마감 폴백 + 기준 시세) ──
function useRecData() {
...
export default function RealtimeChartProbe(props: { width?: number }) {
  ...
  // 변화율: 라이브 캔들 있으면 첫 시가 대비, 없으면 rec change_pct
