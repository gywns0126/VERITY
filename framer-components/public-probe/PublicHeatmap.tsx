...
function sectorOf(s: any): string {
    return (
        (s.peer && s.peer.sector) || (s.overview && s.overview.sector) || "기타"
    )
}
...
function squarify(
    ...
    const scaled = items.map((it) => ({
        ...it,
        area: Math.max(0.0001, (it.value > 0 ? it.value : 0) * scale),
    }))
    let x = X,
        y = Y,
        w = W,
...
export default function PublicHeatmap(props: Props) {
    ...
    const [zoom, setZoom] = useState<{ z: number; tx: number; ty: number }>({
        z: 1,
        tx: 0,
        ty: 0,
    })
    const dragRef = useRef<{
        ...
        ty: number
        moved: boolean
    }>({ on: false, x: 0, y: 0, tx: 0, ty: 0, moved: false })
    ...
    const metricVal = (ticker: string): number | null => {
        ...
        if (metric === "dilution") {
            ...
            return isFinite(v as number) && (v as number) > 0
                ? (v as number)
                : null
        }
        return null
    ...
    const layout = useMemo(() => {
        ...
        const sectorItems = Object.keys(bySec)
            .map((name) => ({
                key: name,
                value: bySec[name].reduce((a, b) => a + b.cap, 0),
                members: bySec[name],
            }))
        ...
        for (const sr of secRects) {
            ...
            const mem = sr.item.members.map((m: any) => ({
                key: m.ticker,
                value: m.cap,
                m,
            }))
            ...
            for (const lf of leaves)
                tiles.push({ ...lf, m: lf.item.m, sector: sr.key })
        }
    ...
    const tabBtn = (m: Metric): CSSProperties => {
        ...
        return {
            ...
            cursor: "pointer",
            fontFamily: FONT,
            padding: narrow ? "7px 11px" : "8px 14px",
            borderRadius: 999,
            fontSize: narrow ? 12 : 13,
            fontWeight: 800,
            flexShrink: 0,
            background: active ? C.vg : C.card,
    ...
    const SKEL_W = [
        34, 22, 22, 14, 30, 16, 18, 12, 24, 16, 14, 10, 20, 12, 16, 10, 14, 22,
        12, 16,
    ]
            ...
            >
                ...
                <div
                    style={{
                        ...
                        overflow: "hidden",
                        background: C.card,
                        cursor: zoom.z > 1 ? "grab" : "default",
                        touchAction: zoom.z > 1 ? "none" : "auto",
                    }}
                    onMouseLeave={() => {
                        setHover(null)
                    ...
                    {layout.tiles.length === 0 ? (
                            ...
                            {SKEL_W.map((wp, i) => (
                                <div
                                    ...
                                    style={{
                                        ...
                                        animation:
                                            "vsrShimmer 1.4s ease-in-out infinite",
                                    }}
                                />
                            ))}
                        </div>
                        ...
                        <>
                                ...
                                return sw > 8 && sh > 8 ? (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            height: Math.max(0, sh - 4),
                                            border: `1px solid ${isDark ? "rgba(255,255,255,0.15)" : "rgba(0,0,0,0.13)"}`,
                                            borderRadius: 8,
                                            boxSizing: "border-box",
                                            pointerEvents: "none",
                                            zIndex: 6,
                            ...
                            {layout.tiles.map((t) => {
                                ...
                                const txt = (
                                    isHover ? col.strongHover : col.strong
                                )
                                    ? C.tileInk
                                    : C.ink
                                return (
                                    <div
                                        ...
                                        style={{
                                            ...
                                            left: tx,
                                            top: ty,
                                            width: Math.max(0, tw - 1.5),
                                            height: Math.max(0, th - 1.5),
                                            background: isHover
                                                ? col.bgHover
                                                : col.bg,
                                            borderRadius: 3,
                                            ...
                                            padding: 2,
                                            boxSizing: "border-box",
                                            border: `1px solid ${isDark ? "rgba(0,0,0,0.25)" : "rgba(255,255,255,0.35)"}`,
                            ...
                            >
                                ...
                                <button
                                    onMouseDown={(e) => e.stopPropagation()}
                                    onClick={() =>
                                        zoomAt(1 / 1.5, chartW / 2, chartH / 2)
                                    }
                                    style={zoomBtnStyle}
                                >
                                    −
                    ...
                    (() => {
                        ...
                        return (
                            ...
                            >
                                ...
                                >
                                    {hover.sector} · 시총{" "}
                                    {capLabel(hover.m.cap)}
                                </div>
                                {hoverRow(
                                    "내부자 순매수",
                                    insiderFmt(insiderMap[hover.m.ticker]),
                                    C
                                )}
                                {hoverRow(
                                    "외국인 5일",
                                    flowFmt(flowMap[hover.m.ticker]),
                                    C
                                )}
                                {hoverRow(
                                    "희석 공시",
                                    dilFmt(forenMap[hover.m.ticker]),
                                    C
                                )}
                                <div
                                    style={{
...
function insiderFmt(e: any): string {
    ...
    return (
        fmtSharesShort(Number(e.net_change)) +
        ` (매수 ${e.buy_n || 0}/매도 ${e.sell_n || 0})`
    )
}
...
function buildSample() {
    const mk = (ticker: string, name: string, sector: string, cap: string) => ({
        ticker,
        name,
        market: "KOSPI",
        facts: { 시가총액: cap },
