 ...
 *
 * 🚨 차별점: 색 = 우리 고유 사실 — 내부자 순매수(DART) / 외국인 수급(네이버 5일) / 희석 공시 빈도(유증·CB).
 * 박스 크기 = 시가총액. 섹터 그룹. 클릭 → 종목 리포트. 줌(+/−·더블클릭·드래그).
 * RULE 7: 자체 점수·등급·verdict 0. 레이아웃 = squarified treemap(순수 JS, 외부 lib 0).
 *   onAccent = 보라(vg) 위 글자색(라이트 흰/다크 짙음).
 *
 * 🚨 2026-07-24 테마 = 자체 내장 CSS 변수(--an-hm-*) 구동. JS 다크 감지 전면 제거 + 헤드 CSS 의존 제거.
 *   <style>{AN_PALETTE} 정적 HTML 정합. 🚨 타일 색 = rgba(var(--an-hm-{up,down,amber}RGB), alpha) — hexA 대체(동적 알파 + var 양립). 되돌리지 말 것.
 */

const LIGHT = {
    bg: "#f2f4f6", card: "#ffffff", ink: "#191f28", sub: "#4e5968", faint: "#8b95a1", line: "#e5e8eb",
    up: "#f04452", down: "#3182f6", amber: "#ff9500", vg: "#6c5ce7", vgS: "#f0edff", vt: "#6c5ce7",
    neutral: "#e9edf1", neutralHover: "#dde2e7", onAccent: "#ffffff", tileInk: "#ffffff",
    skBase: "#e9edf1", skHi: "#f3f5f7",
    secFrame: "rgba(0,0,0,0.13)", secLabelBg: "rgba(255,255,255,0.78)", tileBorder: "rgba(255,255,255,0.35)",
    upRGB: "240,68,82", downRGB: "49,130,246", amberRGB: "255,149,0",
}
const DARK = {
    bg: "#0f1318", card: "#171c23", ink: "#e3e7ec", sub: "#9aa4b1", faint: "#828d9b", line: "#252b34",
    up: "#f04452", down: "#5b9bff", amber: "#ffb454", vg: "#a99bff", vgS: "#241f3a", vt: "#a99bff",
    neutral: "#222a33", neutralHover: "#2b3440", onAccent: "#0f1318", tileInk: "#ffffff",
    skBase: "#222a33", skHi: "#2d3742",
    secFrame: "rgba(255,255,255,0.15)", secLabelBg: "rgba(23,28,35,0.72)", tileBorder: "rgba(0,0,0,0.25)",
    upRGB: "240,68,82", downRGB: "91,155,255", amberRGB: "255,180,84",
}
const FONT = "Pretendard, -apple-system, BlinkMacSystemFont, 'Apple SD Gothic Neo', sans-serif"

// 🎨 팔레트 자체 내장 — LIGHT/DARK 를 CSS 변수(--an-hm-*)로 발행. 정적 HTML 정합. 되돌리지 말 것.
const _ANP = "hm"
const AN_PALETTE =
...
export default function PublicHeatmap(props: Props) {
    ...
    useEffect(() => {
        if (onCanvas) {
            setStocks(SAMPLE.stocks)
            setInsiderMap(SAMPLE.insider)
            setFlowMap(SAMPLE.flow)
            setForenMap(SAMPLE.foren)
            return
        }
        let alive = true
        ...
        jget(stockUrl, (d) => {
            ...
            if (Array.isArray(a)) setStocks(a)
            const ts = d && d._meta && d._meta.generated_at
            if (ts) setAsOf(String(ts))
        })
        jget(insiderUrl, (d) => {
            const a = Array.isArray(d) ? d : d.stocks
            if (!Array.isArray(a)) return
            const m: Record<string, any> = {}
            for (const x of a) if (x && x.ticker) m[String(x.ticker)] = x
            setInsiderMap(m)
        })
        jget(flowUrl, (d) => {
            const fm = d.flows || d
            if (fm && typeof fm === "object") setFlowMap(fm)
        })
        jget(forensicsUrl, (d) => {
            const a = Array.isArray(d) ? d : d.stocks
            if (!Array.isArray(a)) return
    ...
    const metricVal = (ticker: string): number | null => {
        if (metric === "insider") {
            const e = insiderMap[ticker]
            const v = e && Number(e.net_change)
            return isFinite(v as number) && v !== 0 ? (v as number) : null
        }
        if (metric === "flow") {
            const f = flowMap[ticker]
            if (f && f.length) {
        ...
        if (metric === "dilution") {
            const fo = forenMap[ticker]
            const v = fo && Number(fo.dilution_count)
            return isFinite(v as number) && (v as number) > 0 ? (v as number) : null
        }
        return null
    }
    ...
    const layout = useMemo(() => {
        ...
        for (const x of uni) {
            ;(bySec[x.sector] = bySec[x.sector] || []).push(x)
        }
        const sectorItems = Object.keys(bySec)
            .map((name) => ({ key: name, value: bySec[name].reduce((a, b) => a + b.cap, 0), members: bySec[name] }))
    ...
    // 🚨 rgba(var(--an-hm-{up,down,amber}RGB), a) — 동적 알파를 var 색에 적용(hexA 대체, 범용 지원).
    const tileColor = (v: number | null): { bg: string; bgHover: string; strong: boolean; strongHover: boolean } => {
        ...
        let rgb = C.upRGB
        if (activeMetric.diverging) rgb = v >= 0 ? C.upRGB : C.downRGB
        else rgb = C.amberRGB
        return {
            bg: `rgba(${rgb},${a.toFixed(3)})`,
            bgHover: `rgba(${rgb},${aH.toFixed(3)})`,
            strong: a >= 0.5,
            strongHover: aH >= 0.5,
        }
    }
    ...
    const clampPan = (z: number, tx: number, ty: number) => {
        const minX = Math.min(0, chartW - chartW * z),
            minY = Math.min(0, chartH - chartH * z)
    ...
    const tabBtn = (m: Metric): CSSProperties => {
        ...
        return {
            border: "none", cursor: "pointer", fontFamily: FONT, padding: narrow ? "7px 11px" : "8px 14px", borderRadius: 999,
            fontSize: narrow ? 12 : 13, fontWeight: 800, flexShrink: 0,
            background: active ? C.vg : C.card, color: active ? C.onAccent : C.sub, boxShadow: active ? "none" : "0 1px 2px rgba(0,0,0,0.05)",
        }
    }
    const zoomBtnStyle: CSSProperties = {
        width: 26, height: 26, borderRadius: 7, border: "none", cursor: "pointer", background: C.card, color: C.sub,
        fontSize: 15, fontWeight: 800, fontFamily: FONT, boxShadow: "0 1px 3px rgba(0,0,0,0.18)", lineHeight: 1,
    ...
    const legend = () => {
        const steps = activeMetric.diverging
            ? [
                  { v: -1, l: activeMetric.negLabel },
                  { v: -0.4, l: "" },
                  { v: 0, l: "" },
                  { v: 0.4, l: "" },
                  { v: 1, l: activeMetric.posLabel },
              ]
            : [
                  { v: 0.1, l: "적음" },
                  { v: 0.4, l: "" },
    ...
    // 호버 카드 위치 — 커서 기준(2026-07-20). 차트 컨테이너 안에만 = 네브바 침범 0.
    const hoverCardPos = () => {
        const CW = 190,
            CH = 132,
            G = 14
        ...
        <div ref={rootRef} style={wrap}>
            ...
            <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 10, flexWrap: "wrap", marginBottom: 8 }}>
                <div style={{ minWidth: 0 }}>
                    <div style={{ fontSize: 18, fontWeight: 800, letterSpacing: "-0.4px" }}>엣지 히트맵</div>
            ...
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginBottom: 8 }}>
                {metricsShown.map((m) => (
                    <button key={m.key} onClick={() => setMetric(m.key)} style={tabBtn(m)}>
                        {m.tab}
                    </button>
                ))}
            </div>
            ...
            {/* 🚨 되돌리지 말 것 — isolation:"isolate" = 스택 문맥 생성. position:"relative" 만으로는
                문맥이 안 생겨 내부 zIndex(줌 18 · 호버카드 20)가 프레이머 페이지 루트까지 올라가
                관심종목 패널(fixed·z auto)을 뚫는다. 양수 z 는 z:auto 인 요소 전부보다 위라
                DOM 순서로는 못 이긴다 (2026-08-23 PM 신고). */}
            <div ref={chartRef} style={{ position: "relative", isolation: "isolate", width: "100%", height: chartH }}>
                {/* 타일 클립 레이어 (overflow hidden = 줌/팬 타일만 가둠) */}
                <div
                    style={{ position: "absolute", inset: 0, borderRadius: 14, overflow: "hidden", background: C.card, cursor: zoom.z > 1 ? "grab" : "default", touchAction: zoom.z > 1 ? "none" : "auto" }}
                    onMouseLeave={() => {
                        setHover(null)
                    ...
                    onMouseDown={(e) => {
                        ...
                        if (zoom.z <= 1) return
                        dragRef.current = { on: true, x: e.clientX, y: e.clientY, tx: zoom.tx, ty: zoom.ty, moved: false }
                    }}
                    onMouseMove={(e) => {
                        ...
                        const c = clampPan(zoom.z, d.tx + dx, d.ty + dy)
                        setZoom((st) => ({ ...st, tx: c.tx, ty: c.ty }))
                    }}
                    onMouseUp={() => {
                        dragRef.current.on = false
                    }}
                    onTouchStart={(e) => {
                        dragRef.current.moved = false
                        if (zoom.z <= 1 || !e.touches[0]) return
                        dragRef.current = { on: true, x: e.touches[0].clientX, y: e.touches[0].clientY, tx: zoom.tx, ty: zoom.ty, moved: false }
                    }}
                    onTouchMove={(e) => {
                        const d = dragRef.current
                        if (!d.on || !e.touches[0]) return
                        const dx = e.touches[0].clientX - d.x,
                            dy = e.touches[0].clientY - d.y
                        if (Math.abs(dx) + Math.abs(dy) > 3) d.moved = true
                        const c = clampPan(zoom.z, d.tx + dx, d.ty + dy)
                        setZoom((st) => ({ ...st, tx: c.tx, ty: c.ty }))
                    }}
                    ...
                    {layout.tiles.length === 0 ? (
                        <div style={{ position: "absolute", inset: 8, display: "flex", flexWrap: "wrap", gap: 6, alignContent: "flex-start", overflow: "hidden" }}>
                            ...
                            {SKEL_W.map((wp, i) => (
                                <div
                                    key={i}
                                    style={{
                                        width: `calc(${wp}% - 6px)`, height: 70 + ((i * 41) % 110), borderRadius: 4, background: skBase,
                        ...
                        <>
                            ...
                            {layout.sectors.map((s, i) => {
                                const sx = s.x * zoom.z + zoom.tx,
                                    sy = s.y * zoom.z + zoom.ty,
                                    sw = s.w * zoom.z,
                                    sh = s.h * zoom.z
                                return sw > 8 && sh > 8 ? (
                                    <div
                                        key={"sf" + i}
                                        style={{ position: "absolute", left: sx + 2, top: sy + 2, width: Math.max(0, sw - 4), height: Math.max(0, sh - 4), border: `1px solid ${C.secFrame}`, borderRadius: 8, boxSizing: "border-box", pointerEvents: "none", zIndex: 6 }}
                                    />
                                ) : null
                            })}
                            {/* 섹터 라벨 */}
                            {layout.sectors.map((s, i) => {
                                ...
                                return sw > 50 && sh > 22 ? (
                                    <div
                                        key={"sec" + i}
                                        style={{ position: "absolute", left: sx + 4, top: sy + 4, maxWidth: sw - 8, fontSize: narrow ? 9.5 : 10.5, fontWeight: 800, color: C.ink, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis", pointerEvents: "none", letterSpacing: "-0.2px", background: C.secLabelBg, padding: "1px 5px", borderRadius: 5, zIndex: 8 }}
                                    >
                                        {s.name}
                                    </div>
                                ) : null
                            ...
                            {/* 타일 */}
                            {layout.tiles.map((t) => {
                                const tx = t.x * zoom.z + zoom.tx,
                                    ty = t.y * zoom.z + zoom.ty,
                                    tw = t.w * zoom.z,
                                    th = t.h * zoom.z
                                const v = metricVal(t.m.ticker)
                                ...
                                return (
                                    <div
                                        key={t.key}
                                        onClick={() => {
                                            if (!dragRef.current.moved) go(t.m.ticker)
                                        }}
                                        onMouseEnter={(e) => {
                                            const r = chartRef.current ? chartRef.current.getBoundingClientRect() : null
                                            setHover({ ...t, v, mx: r ? e.clientX - r.left : undefined, my: r ? e.clientY - r.top : undefined })
                                        }}
                                        onMouseMove={(e) => {
                                            if (dragRef.current.on || !chartRef.current) return
                                            const r = chartRef.current.getBoundingClientRect()
                                            setHover((h: any) => (h && h.key === t.key ? { ...h, mx: e.clientX - r.left, my: e.clientY - r.top } : h))
                                        }}
                                        role="button"
                                        tabIndex={0}
                                        style={{
                                            ...
                                            display: "flex", flexDirection: "column", justifyContent: "center", alignItems: "center", padding: 2, boxSizing: "border-box",
                                            border: `1px solid ${C.tileBorder}`,
                                            // 호버 = 효과 없이 색만 살짝 진하게(알파 ↑). lift·그림자·외곽선 0.
                                            transition: "background 110ms ease", zIndex: 1,
                                        }}
                                    >
                                        {big && (
                                            <>
                                                {tw > 52 && th > 50 ? (
                                                    bfLogoSrc(t.m.ticker, __lmH, 16) ? (
                                                        <img
                                                            src={bfLogoSrc(t.m.ticker, __lmH, 16)}
                                                            alt=""
                                                            loading="lazy"
                                                            decoding="async"
                                                            width={16}
                                                            ...
                                                            style={{ width: 16, height: 16, borderRadius: 5, marginBottom: 2, objectFit: "cover", display: "block", background: "transparent" }}
                                                        />
                                                    ) : (
                                                        <span
                                                            style={{ width: 16, height: 16, borderRadius: 5, marginBottom: 2, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 10, fontWeight: 800, color: txt, background: col.strong ? "rgba(255,255,255,0.22)" : "rgba(0,0,0,0.06)", boxSizing: "border-box" }}
                                                        >
                                                            {String(t.m.name || t.m.ticker).trim().charAt(0)}
                                                        </span>
                                                    )
                                                ) : null}
                                                <span
                                                    style={{
                                                        fontSize: tw > 130 ? 13 : tw > 80 ? 12 : 10.5, fontWeight: 800, color: txt, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis",
                                                        maxWidth: "100%", letterSpacing: "-0.3px", textShadow: col.strong ? "0 1px 2px rgba(0,0,0,0.35)" : "none",
                                                    }}
                                                >
                                                    {t.m.name}
                                                </span>
                                                {th > 42 && (
                                                    <span style={{ fontSize: 10.5, fontWeight: 700, color: txt, opacity: 0.92, marginTop: 1, textShadow: col.strong ? "0 1px 2px rgba(0,0,0,0.3)" : "none" }}>
                                                        {tileValLabel(metric, v)}
                                                    </span>
                                                )}
                                            </>
                                        )}
                            ...
                            <div style={{ position: "absolute", top: 8, right: 8, display: "flex", flexDirection: "column", gap: 4, zIndex: 18 }}>
                                <button onMouseDown={(e) => e.stopPropagation()} onClick={() => zoomAt(1.5, chartW / 2, chartH / 2)} style={zoomBtnStyle}>
                                    +
                                </button>
                                <button onMouseDown={(e) => e.stopPropagation()} onClick={() => zoomAt(1 / 1.5, chartW / 2, chartH / 2)} style={zoomBtnStyle}>
                                    −
                                </button>
                                {zoom.z > 1.01 && (
                                    <button onMouseDown={(e) => e.stopPropagation()} onClick={resetZoom} style={{ ...zoomBtnStyle, fontSize: 9, height: 18 }}>
                                        1:1
                                    </button>
                                )}
                            </div>
                ...
                {hover &&
                    ...
                    (() => {
                        ...
                        return (
                            <div
                                style={{ position: "absolute", left: cl, top: ct, width: CW, background: C.card, border: `1px solid ${C.line}`, borderRadius: 10, boxShadow: "0 8px 26px rgba(0,0,0,0.22)", padding: "9px 11px", zIndex: 20, pointerEvents: "none", boxSizing: "border-box" }}
                            >
                                <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
                                    {bfLogoSrc(hover.m.ticker, __lmH, 18) ? (
                                        <img
                                            src={bfLogoSrc(hover.m.ticker, __lmH, 18)}
                                            alt=""
                                            loading="lazy"
                                            decoding="async"
                                            width={18}
                                            ...
                                            style={{ width: 18, height: 18, borderRadius: 6, flexShrink: 0, objectFit: "cover", display: "block", background: "transparent" }}
                                        />
                                    ) : (
                                        <span style={{ width: 18, height: 18, borderRadius: 6, flexShrink: 0, display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, fontWeight: 800, color: C.sub, background: C.bg, boxSizing: "border-box" }}>
                                            {String(hover.m.name || hover.m.ticker).trim().charAt(0)}
                                        </span>
                                    )}
                                ...
                                <div style={{ fontSize: 10.5, color: C.faint, fontWeight: 600, marginBottom: 5, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>
                                    {hover.sector} · 시총 {capLabel(hover.m.cap)}
                                </div>
...
function buildSample() {
    ...
    const stocks = [
        mk("005930", "삼성전자", "반도체", "425조"),
        mk("000660", "SK하이닉스", "반도체", "180조"),
        mk("373220", "LG에너지솔루션", "2차전지", "95조"),
        mk("207940", "삼성바이오로직스", "바이오", "70조"),
        mk("005380", "현대차", "자동차", "52조"),
        mk("000270", "기아", "자동차", "44조"),
        mk("068270", "셀트리온", "바이오", "40조"),
        mk("105560", "KB금융", "금융", "38조"),
        mk("055550", "신한지주", "금융", "30조"),
        mk("247540", "에코프로비엠", "2차전지", "14조"),
        mk("035420", "NAVER", "인터넷", "33조"),
        mk("035720", "카카오", "인터넷", "20조"),
        mk("012330", "현대모비스", "자동차", "22조"),
        mk("042700", "한미반도체", "반도체", "11조"),
        mk("034020", "두산에너빌리티", "기계", "13조"),
        mk("009540", "HD한국조선해양", "조선", "12조"),
    ]
    ...
    const flow: Record<string, any[]> = {
        "005930": [{ foreign_net: 230000 }],
        "000660": [{ foreign_net: 145000 }],
        "373220": [{ foreign_net: -90000 }],
        "035420": [{ foreign_net: 60000 }],
        "035720": [{ foreign_net: -40000 }],
        "068270": [{ foreign_net: 28000 }],
    }
    const foren: Record<string, any> = {
        "247540": { ticker: "247540", dilution_count: 12 },
        "034020": { ticker: "034020", dilution_count: 6 },
        "035720": { ticker: "035720", dilution_count: 4 },
        "009540": { ticker: "009540", dilution_count: 3 },
    }
