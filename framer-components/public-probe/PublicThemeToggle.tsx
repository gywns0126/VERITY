...
const THEME_CSS =
    ...
    "--token-a8427b85-996c-4c32-93b4-222935fc2f01:#15c47e;" + // MenuHover
    "}" +
    ...
    "--token-a8427b85-996c-4c32-93b4-222935fc2f01:#15c47e;" +
    "}"
...
const PAL = {
    light: { bg: "#ffffff", border: "#e5e8eb", icon: "#4e5968" },
    dark: { bg: "#171c23", border: "#2b3138", icon: "#f2f4f6" },
}

function SunIcon(props: { size: number; color: string }) {
    const s = props.size
    return (
        <svg width={s} height={s} viewBox="0 0 24 24" fill="none" stroke={props.color} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden="true">
...
function MoonIcon(props: { size: number; color: string }) {
    const s = props.size
    return (
        <svg width={s} height={s} viewBox="0 0 24 24" fill={props.color} aria-hidden="true">
...
export default function PublicThemeToggle(props: Props) {
    ...
    const pal = theme === "dark" ? PAL.dark : PAL.light
    ...
    return (
        ...
        >
            {theme === "dark" ? <MoonIcon size={iconSize} color={pal.icon} /> : <SunIcon size={iconSize} color={pal.icon} />}
        </button>
    )
}
