 ...
 * 🚨 수수료 = 숫자 assert 금지 → 각 사 공식 고지 deep-link. 신뢰 정성사실만 노출.
 * 🚨 이벤트 = 진행 중 사실(기간 포함) · 권유 아님. 로고 = 증권사 공식 도메인 Clearbit + 구글 파비콘 fallback.
 *
 * 🚨 2026-07-24 테마 = 자체 내장 CSS 변수(--an-brk-*) 구동. JS 다크 감지 전면 제거 + 헤드 CSS 의존 제거.
 *   <style>{AN_PALETTE} 정적 HTML 정합. 서브컴포넌트 C prop = var Record. 되돌리지 말 것.
 */
...
const LIGHT = {
    bg: "#ffffff", card: "#ffffff", sub: "#f2f4f6", text: "#191f28", subtext: "#6b7280",
    faint: "#8b95a1", border: "#e5e8eb", accent: "#6c5ce7", chipBg: "#f2f4f6",
    good: "#0ca678", goodBg: "#e7f7f0", skBase: "#e9edf1", skHi: "#f3f5f7",
}
const DARK = {
    bg: "#171c23", card: "#1e242c", sub: "#222933", text: "#f2f4f6", subtext: "#9aa4b1",
    faint: "#6b7682", border: "#2b3138", accent: "#a99bff", chipBg: "#222933",
    good: "#34e08a", goodBg: "#16302a", skBase: "#222a33", skHi: "#2d3742",
}

// 🎨 팔레트 자체 내장 — LIGHT/DARK 를 CSS 변수(--an-brk-*)로 발행. 정적 HTML 정합. 되돌리지 말 것.
const _ANP = "brk"
const AN_PALETTE =
...
function splitBrokers(best: string): string[] {
    return String(best || "").split(/[/,·、]| 및 | 또는 /).map((s) => s.trim()).filter(Boolean)
}
...
// 국내수수료 요점만 — 첫 %(복합 대비). 전문은 hover. 무료 이벤트 대응.
...
function BrokerLogo(props: { name: string; size: number; C: any }) {
    ...
    if (!primary) {
        return (
            <span style={{ width: size, height: size, borderRadius: 7, background: C.chipBg, color: C.subtext, fontSize: Math.round(size * 0.46), fontWeight: 800, display: "inline-flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
                {initial}
            </span>
        )
    }
    return (
        <img
            ...
            style={{ width: size, height: size, borderRadius: 7, objectFit: "contain", background: "#ffffff", flexShrink: 0 }}
        />
    )
}

function Chip(props: { label: string; value: string; C: any }) {
    const { label, C } = props
    const v = clean(props.value)
    const pos = /지원|있음|가능|포함|제공/.test(v) && !/미지원|불가|없음|불가능|미제공/.test(v)
    return (
        <span style={{ display: "inline-flex", alignItems: "center", gap: 5, background: pos ? C.goodBg : C.chipBg, borderRadius: 8, padding: "4px 9px" }}>
...
/* 라벨 + 사실 텍스트 한 줄. 3줄 클램프. */
function InfoLine(props: { label: string; value: string; C: any }) {
    ...
    return (
        <div style={{ display: "flex", gap: 8, fontSize: 11.5, lineHeight: 1.45, alignItems: "baseline" }}>
            ...
            <span style={{ color: C.subtext, fontWeight: 500, overflowWrap: "anywhere", display: "-webkit-box", WebkitLineClamp: 3, WebkitBoxOrient: "vertical", overflow: "hidden" }}>
                {value}
            </span>
        </div>
    )
...
/* 토스식 shimmer 스켈레톤 */
function LoadingSkeleton(props: { C: any }) {
    const { C } = props
    const base = C.skBase
    const hi = C.skHi
    ...
    const bar = (w: number | string, h: number, mt?: number): React.CSSProperties => ({ ...shimmer, width: w, height: h, marginTop: mt || 0 })
    ...
    return (
        <div>
            ...
            <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 300px), 1fr))", gap: 12, alignItems: "start", paddingTop: 6 }}>
                {rows.map((i) => (
                    <div key={i} style={{ height: 210, background: C.card, borderRadius: 16, padding: "16px 17px", boxSizing: "border-box", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
...
export default function PublicBrokerGuide(props: Props) {
    ...
    // 🚨 2026-07-19 재테마 — root 흰 카드 감싸기 제거. 표준 회색페이지 규약:
    //   root=투명·자연 흐름 + 헤더·아이템을 개별 흰 카드로. props.height = 최소 높이 floor.
    const wrap: React.CSSProperties = {
        width: "100%", maxWidth: 1180, marginLeft: "auto", marginRight: "auto",
        minHeight: props.height || 720, background: "transparent", display: "flex",
        ...
        <div style={wrap}>
            ...
            <div style={{ padding: "16px 14px 12px" }}>
                <div style={{ display: "flex", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
                    ...
                    {guide && guide.as_of ? (
                        <span style={{ fontSize: 11, fontWeight: 600, color: C.faint, marginLeft: "auto" }}>기준 {dateOnly(guide.as_of)}</span>
                    ) : null}
                </div>
                <div style={{ marginTop: 13, display: "inline-flex", gap: 6 }}>
                    {tabs.map((t) => {
                        ...
                        return (
                            <button
                                ...
                                style={{ border: "none", cursor: "pointer", background: active ? "#6c5ce7" : "transparent", color: active ? "#ffffff" : C.subtext, fontWeight: active ? 700 : 600, fontSize: 13, padding: "7px 15px", borderRadius: 9, transition: "all 140ms ease" }}
                            >
                                {t.label}
                            </button>
                        )
                    ...
                    {asArray(guide.citations).length ? (
                        <div style={{ marginTop: 5, display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
                            ...
                            {asArray(guide.citations).slice(0, 6).map((u, i) => (
                                <a key={i} href={String(u)} target="_blank" rel="noopener noreferrer" style={{ fontSize: 10.5, color: C.accent, fontWeight: 600, textDecoration: "none" }}>
                                    [{i + 1}]
                                </a>
                            ))}
                        </div>
...
function BrokerPills(props: { best: string; C: any }) {
    const { best, C } = props
    ...
    return (
        <div style={{ display: "flex", flexWrap: "wrap", gap: 6 }}>
            {names.map((bn, i) => (
                <span key={i} style={{ display: "inline-flex", alignItems: "center", gap: 6, background: C.sub, borderRadius: 9, padding: "4px 10px 4px 4px" }}>
...
function TradeTypeView(props: { items: TradeType[]; C: any; cardH: number }) {
    ...
    return (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 340px), 1fr))", gap: 10, alignItems: "start", paddingTop: 6 }}>
            {items.map((it, i) => (
                <div key={i} style={{ minHeight: cardH, background: C.card, borderRadius: 16, padding: "15px 16px", boxSizing: "border-box", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                    ...
                    {isEmptyVal(it.best) ? (
                        ...
                        <>
                            ...
                            <div style={{ marginTop: 10, fontSize: 12.5, color: C.subtext, fontWeight: 500, lineHeight: 1.55, overflowWrap: "anywhere" }}>
                                {clean(it.reason)}
                            </div>
                        </>
                    )}
...
/* 대표 수수료 1칸 — 라벨 + 값(공식 출처 링크 ↗). 상세는 title(hover). */
function FeeTile(props: { label: string; value: string; url: string; basis?: string; C: any }) {
    const { label, value, url, basis, C } = props
    const v = clean(value)
    const numStyle: React.CSSProperties = { fontSize: 14.5, fontWeight: 700, letterSpacing: "-0.01em", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }
    return (
...
function BrokerTable(props: { brokers: Broker[]; C: any; cardH: number }) {
    ...
    return (
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(min(100%, 320px), 1fr))", gap: 12, alignItems: "start", paddingTop: 6 }}>
            {brokers.map((b, i) => {
                ...
                return (
                    <div key={i} style={{ minHeight: cardH, background: C.card, borderRadius: 16, padding: "16px 17px", boxSizing: "border-box", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                        {/* 헤더: 로고 + 이름 */}
                        ...
                        {/* 대표 수수료 — 하이브리드: 공식 출처 링크가 붙은 값만 tile. 없으면 공식 고지 링크 fallback. */}
                        {feeTiles.length ? (
                            <div style={{ marginBottom: 11 }}>
                                ...
                                <div style={{ fontSize: 9.5, color: C.faint, fontWeight: 500, marginTop: 6 }}>온라인 위탁 기준 · 값 클릭 시 공식 출처</div>
                            </div>
                        ) : feeUrl ? (
                            <a href={feeUrl} target="_blank" rel="noopener noreferrer" style={{ display: "flex", alignItems: "center", justifyContent: "space-between", gap: 8, background: C.sub, borderRadius: 10, padding: "10px 12px", marginBottom: 11, textDecoration: "none" }}>
                                <div>
...
addPropertyControls(PublicBrokerGuide, {
    dark: { type: ControlType.Boolean, title: "다크(미사용)", defaultValue: false },
