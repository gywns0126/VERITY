 ...
 * 검색 universe = stock_report_public.json. RULE 7 — 종목 검색 도구일 뿐, 점수·추천 0.
 * 종목 공유 = ?q 1순위 + localStorage `verity_last_ticker` 폴백(리포트↔결정 페이지 토글 시 종목 유지).
 */
...
export default function PublicTickerPicker(props: Props) {
    ...
    const pick = (tk: string) => {
        ...
        try { window.localStorage.setItem(LAST_TK_KEY, tk) } catch {}
            ...
            <div style={{ background: C.card, borderRadius: 14, padding: "13px 15px", boxShadow: "0 1px 3px rgba(0,0,0,0.04)" }}>
                ...
                {matches.length > 0 && (
                    <div style={{ marginTop: 6, border: `1px solid ${C.line}`, borderRadius: 11, padding: 4, maxHeight: 260, overflowY: "auto" }}>
                        {matches.map((m) => (
                            <div key={m.ticker} onClick={() => pick(m.ticker)} role="button" tabIndex={0}
