...
const GLOSSARY: Record<string, string> = {
    유상증자:
        "회사가 새 주식을 발행해 투자자에게 돈을 받고 파는 것. 자금은 들어오지만 주식 수가 늘어 기존 주주 지분이 옅어져요.",
    무상증자:
        "기존 주주에게 공짜로 새 주식을 나눠주는 것. 회사 가치는 그대로라 주당 가격이 그만큼 조정돼요.",
    전환사채:
        "회사가 빌린 돈을 정해진 가격에 주식으로 바꿀 수 있는 채권(CB). 전환되면 주식 수가 늘어요.",
    신주인수권:
        "정해진 가격에 새 주식을 살 수 있는 권리(BW). 행사되면 주식 수가 늘어요.",
    공급계약:
        "제품·서비스를 납품하기로 맺은 계약. 규모가 매출 대비 크면 향후 실적 기대 요인이에요.",
    단일판매:
        "한 건의 큰 판매·공급 계약. 매출 대비 규모가 크면 중요한 사실이에요.",
    자기주식:
        "회사가 자기 회사 주식을 사거나 보유하는 것. 매입하면 시중 주식 수가 줄어 보통 주주에 우호적이에요.",
    자사주: "회사가 보유·매입하는 자기 회사 주식. 매입하면 시중 주식 수가 줄어 보통 주주에 우호적이에요.",
    최대주주:
        "회사 지분을 가장 많이 가진 주주. 이들의 매수·매도는 회사 내부 사정을 비추는 신호예요.",
    대량보유:
        "주식을 5% 이상 보유한 주주의 보유·변동 신고. 큰손의 매수·매도 흐름이 드러나요.",
        ...
        disclosures: [
            {
                ...
                is_correction: false,
                filer: "에코프로비엠",
                source_url: DART + "20260616000412",
            },
            {
                title: "단일판매ㆍ공급계약체결",
                label: "주요사항",
                ...
                is_correction: false,
                filer: "에코프로비엠",
                source_url: DART + "20260609000231",
            },
            {
                title: "[기재정정]분기보고서",
                label: "정정공시",
                ...
                is_correction: true,
                filer: "에코프로비엠",
                source_url: DART + "20260602000118",
            },
        ],
    },
    {
        ...
        disclosures: [
            {
                ...
                is_correction: false,
                filer: "티로우프라이스",
                source_url: DART + "20260612000309",
            },
        ],
    },
]
...
function DisclosureFeedSkeleton({ C, isDark }: { C: any; isDark: boolean }) {
    ...
    const sk = (
        ...
        animation: "vdfShimmer 1.4s ease-in-out infinite",
        flexShrink: 0,
    })
    return (
        <>
...
export default function PublicDisclosureFeed(props: Props) {
    ...
    const shown = useMemo(() => {
        ...
        pinned.sort(
            (a, b) =>
                (widx[String(a.ticker)] ?? 0) - (widx[String(b.ticker)] ?? 0)
        )
        return pinned.concat(rest).slice(0, max)
    }, [filtered, watchKey, maxStocks, query])

    const watchSet = useMemo(
        () => new Set(watchKey ? watchKey.split(",").filter(Boolean) : []),
        [watchKey]
    )
    ...
    const goStock = (ticker: string) => {
        ...
        window.location.href =
            p + "?q=" + encodeURIComponent(String(ticker || "").trim())
    }
    ...
    return (
        <div ref={rootRef} style={wrap}>
            ...
            >
                {!loaded && skelVisible && (
                    <DisclosureFeedSkeleton C={C} isDark={themeDark} />
                )}
                    ...
                    return (
                        ...
                        >
                            ...
                            {it.disclosures.map((d, i) => {
                                ...
                                const chipMeaning = corr
                                    ? LABEL_PLAIN["정정공시"]
                                    : LABEL_PLAIN[d.label] || d.label
                                return (
                                    <div
                                        key={i}
                                        style={{
            ...
            >
                공시 사실·일정만 · 색 표시(희석·우호·주의)는 공시 유형의 일반적
                성격 · 원문은 DART 전자공시
            </div>
        </div>
    )
