 ...
 * 도움말 = 항상 표시(용어 점선밑줄·분류칩). PC(hover) 커서 / 모바일(touch) 탭. 툴팁 clamp.
 * 관심종목 핀: 로그인 시 Supabase watchgroups(단일 소스) 상단 고정 · 미로그인 시 localStorage 폴백.
 *   onAccent = 브랜드 보라(vg) 위 글자색. 라이트=흰색/다크=짙은색(2026-06-21 가독성).
 * 🚨 SAMPLE = canvas 프리뷰 전용. 라이브 초기 state = 빈 배열(fetch 실패 시 가짜 공시 오노출 차단).
 *
 * 🚨 2026-07-24 테마 = 자체 내장 CSS 변수(--an-vdf-*) 구동. JS 다크 감지 전면 제거 + 헤드 CSS 의존 제거.
 *   <style>{AN_PALETTE} 정적 HTML 정합. 🚨 검색 아이콘 SVG stroke = style 로 넘김(var, 자식 상속). DART 버튼 = onAccent 토큰(플립). 되돌리지 말 것.
 */

const LIGHT = {
    bg: "#f2f4f6", card: "#ffffff", ink: "#191f28", sub: "#4e5968", faint: "#8b95a1", line: "#e5e8eb",
    red: "#f04452", redS: "#fff0f1", amber: "#ff9500", amberS: "#fff6e9", blue: "#3182f6", blueS: "#eef4ff",
    green: "#15c47e", greenS: "#eafaf3", vg: "#6c5ce7", vgS: "#f0edff", vt: "#6c5ce7", vtS: "#f0edff",
    tipBg: "#191f28", tipFg: "#ffffff", onAccent: "#ffffff", skBase: "#e9edf1", skHi: "#f3f5f7",
}
const DARK = {
    bg: "#0f1318", card: "#171c23", ink: "#e3e7ec", sub: "#9aa4b1", faint: "#828d9b", line: "#252b34",
    red: "#f04452", redS: "#2a1a1d", amber: "#ff9500", amberS: "#2a2113", blue: "#5b9bff", blueS: "#152031",
    green: "#34e08a", greenS: "#0f241c", vg: "#a99bff", vgS: "#241f3a", vt: "#a99bff", vtS: "#241f3a",
    tipBg: "#222a33", tipFg: "#e3e7ec", onAccent: "#0f1318", skBase: "#222a33", skHi: "#2d3742",
}
...
// 🎨 팔레트 자체 내장 — LIGHT/DARK 를 CSS 변수(--an-vdf-*)로 발행. 정적 HTML 정합. 되돌리지 말 것.
const _ANP = "vdf"
const AN_PALETTE =
...
// 공시 제목 → 유형 성격(사실 기반). dilution=희석 · favor=우호 · alert=주의. 우선순위 alert > dilution > favor.
...
function classifyTone(title: string): Tone | "" {
    const t = title || ""
    for (const g of TONE_KW) {
        if (g.kw.some((k) => t.includes(k))) return g.key
    }
    return ""
}
...
/* 로딩 스켈레톤 — 토스식 shimmer. 실제 레이아웃(종목 카드) 미리 그림. */
function DisclosureFeedSkeleton({ C }: { C: any }) {
    const base = C.skBase
    const hi = C.skHi
...
export default function PublicDisclosureFeed(props: Props) {
    ...
    // SAMPLE 은 canvas 프리뷰 전용 — 라이브는 빈 배열로 시작해 fetch 로만 채운다.
    ...
    // 관심종목 = 로그인 시 Supabase watchgroups(단일 소스) · 미로그인 시 localStorage 폴백.
    useEffect(() => {
        if (onCanvas) return
        let alive = true
    ...
    const filtered = useMemo(() => {
        ...
        if (!q) return items
        return items.filter(
            (it) => String(it.name || "").toLowerCase().includes(q) || String(it.ticker || "").toLowerCase().includes(q)
        )
    }, [items, query])
    ...
    const shown = useMemo(() => {
        ...
        const rest: FeedItem[] = []
        filtered.forEach((it) => {
            ;(wset.has(String(it.ticker)) ? pinned : rest).push(it)
        })
    ...
    const tipStyle = (): CSSProperties => ({
        ...
        width: tipBox.width, background: C.tipBg, color: C.tipFg, borderRadius: 12, padding: "11px 13px",
        fontSize: 12.5, fontWeight: 500, lineHeight: 1.55, letterSpacing: "-0.1px",
        boxShadow: "0 6px 20px rgba(0,0,0,0.18)", whiteSpace: "normal", textAlign: "left",
    })

    const hoverProps = (id: string) =>
        hoverCapable ? { onMouseEnter: (e: any) => openTipAt(e, id), onMouseLeave: () => setOpenTip("") } : {}
            ...
            parts.push(
                <span key={tipId} style={{ position: "relative", display: "inline" }}>
                    <span
                        ...
                        {...hoverProps(tipId)}
                        style={{ borderBottom: `1px dashed ${C.vt}`, cursor: "help" }}
                    >
                        {hitKey}
                    </span>
                    {isOpen && (
    ...
    return (
        <div ref={rootRef} style={wrap}>
            ...
            <div style={{ marginBottom: 4 }}>
                <div style={{ fontSize: 18, fontWeight: 800, letterSpacing: "-0.4px" }}>공시 속보</div>
                <div style={{ fontSize: 12, color: C.faint, fontWeight: 600, marginTop: 3 }}>공시-first (DART 원천) · 관심종목 공시는 위로</div>
            </div>
            ...
            <div style={{ marginTop: 12, position: "relative" }}>
                <svg
                    ...
                    strokeWidth="2.4"
                    strokeLinecap="round"
                    style={{ position: "absolute", left: 12, top: "50%", transform: "translateY(-50%)", pointerEvents: "none", stroke: C.faint }}
                >
                ...
                <input
                    ...
                    style={{ width: "100%", boxSizing: "border-box", border: "none", background: C.card, color: C.ink, borderRadius: 12, padding: "12px 34px 12px 38px", fontSize: 13.5, fontFamily: FONT, outline: "none", WebkitAppearance: "none" }}
                />
                {query && (
                    <span
                        role="button"
                        tabIndex={0}
                        onClick={() => setQuery("")}
                        style={{ position: "absolute", right: 11, top: "50%", transform: "translateY(-50%)", color: C.faint, fontSize: 15, fontWeight: 700, cursor: "pointer", lineHeight: 1 }}
                    >
                        ×
                    </span>
                )}
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 10, marginTop: 12 }}>
                {!loaded && skelVisible && <DisclosureFeedSkeleton C={C} />}
                ...
                {shown.map((it) => {
                    ...
                    return (
                        <div
                            key={it.ticker}
                            style={{ background: C.card, borderRadius: 16, padding: `${cardPadV}px ${cardPadH}px`, boxShadow: "0 1px 3px rgba(0,0,0,0.04)", border: pinned ? `1px solid ${C.vtS}` : "1px solid transparent" }}
                        >
                            {/* 종목명 — 탭하면 전체 리포트 */}
                            <div
                            ...
                            >
                                {pinned && (
                                    <span style={{ flexShrink: 0, fontSize: 10, fontWeight: 800, color: C.vt, background: C.vtS, padding: "2px 7px", borderRadius: 6 }}>관심</span>
                                )}
                                ...
                                return (
                                    <div key={i} style={{ borderTop: i === 0 ? `1px solid ${C.line}` : "none" }}>
                                        ...
                                        >
                                            ...
                                            <div style={{ display: "flex", flexDirection: "column", gap: 5, flexShrink: 0, alignSelf: "flex-start", alignItems: "flex-start" }}>
                                                <span style={{ position: "relative", display: "inline-block" }}>
                                                    <span
                                                        ...
                                                        onClick={(e) => {
                                                            e.stopPropagation()
                                                            if (chipOpen) setOpenTip("")
                                                            else openTipAt(e, chipId)
                                                        }}
                                                        {...hoverProps(chipId)}
                                            ...
                                            <span style={{ flexShrink: 0, alignSelf: "center", fontSize: 13, color: C.faint, fontWeight: 700, transform: opened ? "rotate(90deg)" : "none", transition: "transform 0.12s" }}>
                                                ›
                                            </span>
                                        </div>
                                        {opened && (
