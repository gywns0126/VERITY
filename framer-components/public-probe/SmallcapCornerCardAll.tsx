...
/**
 * 소형주 코너 (통합) — 🇰🇷 한국 / 🇺🇸 미국 국기 토글. KR+US 둘 다.
 * 🚨 RULE 7 — 점수·등급·순위·verdict 0. 공시/재무 사실. LLM 0(RULE 6). data: smallcap_corner_filters.json.
 *
 * 🚨 2026-07-24 테마 = 자체 내장 CSS 변수(--an-scc-*) 구동. JS 다크 감지 전면 제거 + 헤드 CSS 의존 제거.
 *   <style>{AN_PALETTE} 정적 HTML 정합. 되돌리지 말 것.
 */

const LIGHT = {
    bg: "#f2f4f6", card: "#ffffff", ink: "#191f28", sub: "#4e5968", faint: "#8b95a1", line: "#e5e8eb",
    red: "#f04452", redSoft: "#fff0f1", amber: "#ff9500", amberSoft: "#fff6e9", blue: "#3182f6", blueSoft: "#eef4ff",
    green: "#15c47e", greenSoft: "#eafaf3", violet: "#6c5ce7", violetSoft: "#f0edff", skBase: "#e9edf1", skHi: "#f3f5f7",
}
const DARK = {
    bg: "#16181d", card: "#1e2128", ink: "#f0f2f5", sub: "#b0b8c1", faint: "#6b7684", line: "#2b2f37",
    red: "#ff6b76", redSoft: "#3a1f22", amber: "#ffb340", amberSoft: "#3a2c14", blue: "#5a9cff", blueSoft: "#1b2740",
    green: "#3ddc97", greenSoft: "#16322a", violet: "#a98bff", violetSoft: "#2a2440", skBase: "#222a33", skHi: "#2d3742",
}
const FLAG = "https://hatscripts.github.io/circle-flags/flags/"

// 🎨 팔레트 자체 내장 — LIGHT/DARK 를 CSS 변수(--an-scc-*)로 발행. 정적 HTML 정합. 되돌리지 말 것.
const _ANP = "scc"
const AN_PALETTE =
...
const LENS: Record<string, "opp" | "risk"> = {
    neglected_quality: "opp", smallcap_dilution: "risk", smallcap_distress: "risk",
    clean_fin_risky_disc: "risk", accounting_red_flag: "risk",
}
const LENS_META: Record<"opp" | "risk", { label: string }> = {
    opp: { label: "기회 · 숨은 우량주 발굴" }, risk: { label: "위험 · 피할 것 거르기" },
}
...
const MARKETS: Record<"kr" | "us", MarketCfg> = {
    kr: {
        ...
        subtitle: "시총 300~3000억 · 애널리스트·기관이 안 보는 코너. 사실·패턴만.", sigLabel: "공시",
        factBits: (f) => {
            const m: string[] = []
    ...
    us: {
        ...
        subtitle: "시총 $50M~$5B · sell-side 가 안 보는 미국 소형주. SEC 8-K 사실·패턴만.", sigLabel: "8-K",
        factBits: (f) => {
            const m: string[] = []
...
function FactRow(props: { t: Ticker; C: any; market: "kr" | "us"; cfg: MarketCfg; reportPath: string }) {
        ...
        <div style={{ padding: "10px 2px", borderTop: "1px solid " + C.line }}>
            <div style={{ display: "flex", alignItems: "baseline", justifyContent: "space-between", gap: 8, marginBottom: 4 }}>
                <div style={{ display: "flex", alignItems: "baseline", gap: 6, minWidth: 0, flexWrap: "wrap" }}>
                    {url ? (
                        <a href={url} target="_blank" rel="noopener noreferrer" title={t.name + " 분석"}
                            style={{ fontSize: 14, fontWeight: 700, color: C.violet, letterSpacing: -0.2, textDecoration: "none" }}>
                            {t.name} ↗
                        </a>
                    ) : (
...
export default function SmallcapCornerCardAll(props: {
    width?: number; dark?: boolean; market?: string
    krReportPath?: string; usReportPath?: string; krScreenerPath?: string; usScreenerPath?: string
}) {
    ...
    useEffect(() => {
        ...
        fetch(cfg.url + "?t=" + Date.now())
            ...
            .then((j) => {
                if (!alive) return
                setData(j)
                try { sessionStorage.setItem(cfg.cacheKey, JSON.stringify(j)) } catch (e) {}
            })
            .catch((e) => {
                if (!alive) return
    ...
    const flagBtn = (mk: "kr" | "us") => {
        ...
        return (
            <div onClick={() => { if (mk !== market) setMarket(mk) }}
                style={{ cursor: "pointer", display: "flex", alignItems: "center", gap: 6, padding: "5px 11px 5px 7px", borderRadius: 999, background: active ? C.violet : C.card, color: active ? C.bg : C.sub, fontSize: 12.5, fontWeight: 800, letterSpacing: -0.2 }}>
    ...
    const filterCard = (flt: any, i: number, tone: { c: string; bg: string }) => {
        ...
        return (
            <div key={i} style={{ background: C.card, borderRadius: 20, padding: 16, marginBottom: 12, boxShadow: "0 1px 3px rgba(0,0,0,0.12)" }}>
                ...
                {tickers.length > 0 ? (
                    <div onClick={() => setOpen(isOpen ? -1 : i)}
                        style={{ cursor: "pointer", marginTop: 12, fontSize: 12.5, fontWeight: 700, color: C.violet, display: "flex", alignItems: "center", justifyContent: "center", gap: 5, padding: "8px", borderRadius: 10, background: isOpen ? C.violetSoft : C.bg }}>
                        종목 {tickers.length}개 보기 {isOpen ? "▴" : "▾"}
                    </div>
                ) : null}
                {isOpen ? (
                    <div style={{ marginTop: 4 }}>
                        {tickers.slice(0, 8).map((tk: Ticker, j: number) => (
                            <FactRow key={j} t={tk} C={C} market={market} cfg={cfg} reportPath={reportPath} />
                        ))}
                        {tickers.length > 8 ? (
                            <a href={screenerPath + "?filter=" + flt.key} target="_blank" rel="noopener noreferrer"
                                style={{ display: "block", textAlign: "center", fontSize: 12.5, fontWeight: 700, color: C.violet, padding: "11px 0 3px", textDecoration: "none" }}>
        ...
        <div style={shell}>
            ...
            <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", padding: "2px 2px 8px", gap: 8 }}>
                ...
                <div style={{ fontSize: 12, color: C.faint, fontWeight: 600 }}>
                    {meta.universe_n ? meta.universe_n.toLocaleString() + "종목" : data ? "사실 필터" : ""}
                </div>
            </div>
