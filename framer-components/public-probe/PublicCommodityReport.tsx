...
export default function PublicCommodityReport(props: Props) {
    ...
    useEffect(() => {
        if (!item?.macroKey) { setMacro(null); return }
        let alive = true
        ...
        return () => { alive = false }
    }, [item?.macroKey, props.macroUrl])
    if (!item) return null
    const quote = item.macroKey ? (macro?.macro?.[item.macroKey] || macro?.[item.macroKey]) : null
    const generatedAt = quote?.as_of || macro?.collected_at || macro?._meta?.generated_at || macro?.generated_at
    const value = Number(quote?.value), change = Number(quote?.change_pct), source = String(quote?.source || "출처 확인 중")
    ...
    return <div style={{ width: "100%", fontFamily: FONT, color: C.ink }}><style>{PALETTE}</style><main style={{ maxWidth: 1100, margin: "0 auto", display: "grid", gap: 14 }}>
        <section style={card}><div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 16, flexWrap: "wrap" }}><div><div style={{ color: C.blue, fontSize: 12, fontWeight: 850 }}>{item.group} · 선물 연속물</div><h1 style={{ margin: "5px 0 0", fontSize: 25, letterSpacing: "-.6px" }}>{item.name}</h1><div style={{ color: C.faint, fontSize: 12, fontWeight: 700, marginTop: 5 }}>{item.symbol} · {item.exchange} · {item.unit}</div></div><div style={{ textAlign: "right" }}>{hasValue ? <><div style={{ fontSize: 25, fontWeight: 900 }}>{value.toLocaleString("ko-KR", { maximumFractionDigits: 3 })}</div>{hasChange ? <div style={{ marginTop: 4, color: change > 0 ? C.red : change < 0 ? C.blue : C.sub, fontSize: 13, fontWeight: 850 }}>{change > 0 ? "+" : ""}{change.toFixed(2)}%</div> : null}</> : <div style={{ color: C.faint, fontSize: 12, fontWeight: 750 }}>가격 데이터 미연결</div>}<div style={{ color: C.faint, fontSize: 10.5, marginTop: 5 }}>{source} · {fmtTimestamp(generatedAt)} · 지연 가능</div></div></div><div style={{ marginTop: 16, borderRadius: 16, background: C.blueS, padding: "13px 15px", color: C.sub, fontSize: 12.5, lineHeight: 1.65, fontWeight: 650 }}>지금 보고 있는 값은 특정 만기 계약이 아니라 여러 만기를 이어 만든 연속물입니다. 실제 ETF·ETN 수익률은 환율, 운용보수, 추적오차와 롤오버 영향으로 달라질 수 있어요.</div></section>
