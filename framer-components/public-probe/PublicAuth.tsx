 ...
 * 로그인 성공 후 = URL ?next=/path 우선, 없으면 afterLoginPath prop.
 * 🚨 복귀 보존(2026-07-13): OAuth redirect_to 에 window.location.search 포함 → 구글 로그인도 ?next 살아남아
 *   원래 페이지(예: 둥지 /nest)로 복귀. (이전엔 origin+pathname 만이라 구글 시 ?next 소실 → 홈 튕김.)
...
// 현재 페이지(경로+쿼리) — OAuth 복귀 시 ?next 등 쿼리 보존용.
function currentBack(): string {
...
export default function AlphaNestAuth(props: Props) {
    ...
    const [themeDark, setThemeDark] = useState<boolean>(() => (RenderTarget.current() === RenderTarget.canvas ? !!dark : false))
    ...
    useEffect(() => {
        ...
        const consumeHash = async () => {
            ...
            try {
                ...
                // 해시만 제거 — 쿼리(?next)는 보존해 아래 resolveNext 가 원래 페이지로 복귀.
    ...
    const wrap: CSSProperties = { width: "100%", height: "100%", minHeight: "100%", fontFamily: FONT, display: "flex", alignItems: "center", justifyContent: "center", boxSizing: "border-box", color: C.ink, background: C.bg }
...
addPropertyControls(AlphaNestAuth, {
    ...
    redirectUrl: { type: ControlType.String, title: "OAuth Redirect URL", defaultValue: "", placeholder: "구글 복귀 URL (비우면 현재 페이지+쿼리)" },
