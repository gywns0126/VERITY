...
2. 출처 표기는 실제로 사용한 데이터 블록 이름 그대로만 쓴다: (출처: 리포트 사실) (출처: 분기 재무) (출처: 내부자 거래) (출처: 공시 이벤트). 다른 출처명을 지어내지 마라.
3. 데이터가 비어 있는 섹션은 "확인된 데이터 없음"이라고 쓰고 넘어가라. 추정으로 채우지 마라.
4. 리스크 항목이 데이터에 없으면 "수집 범위 내 확인된 리스크 공시 없음"이라고 써라. "리스크 요인이 없다"는 단정 금지 — 데이터 부재는 리스크 부재가 아니다.
5. "지켜볼 것"은 데이터에 있는 구체 항목(공시 이벤트·내부자 흐름·재무 변화·일정)만 연결해 써라. "동향을 주시할 필요가 있다" 같은 일반론 금지. 연결할 구체 항목이 없으면 "특기할 관찰 포인트 없음"이라고 써라.
6. 형식: 아래 5개 섹션 제목을 정확히 사용, 섹션당 2~4문장, 전체 700자 이내.
## 한눈에
## 재무 흐름
## 수급·내부자
## 공시·리스크
...
GEMINI_FALLBACK_MODEL = os.environ.get("GEMINI_FALLBACK_MODEL", "gemini-2.5-flash").strip() or "gemini-2.5-flash"


def _complete(text: str) -> bool:
    """불완전 출력 가드 — 섹션 5개 요구 형식 중 4개 미만이면 잘린 것(캐시 오염 차단)."""
    return bool(text) and text.count("## ") >= 4


def _generate(grounding: str):
    """flash-lite 503(수요 폭주) 실측 빈발 — 재시도 1회 + 폴백 모델 1회 (maxDuration 60 내).
    thinking_budget=0 = 2.5-flash 계열 thinking 토큰이 출력 예산 잠식 방지 (truncation 실측 결함)."""
    ...
    for i, model in enumerate(attempts):
        ...
        try:
            ...
            if _complete(text):
                return text
            _logger.warning("gemini %s 시도 %d 불완전 출력(len=%d) — 재시도", model, i + 1, len(text))
        except Exception as e:
            last_err = e
            _logger.warning("gemini %s 시도 %d 실패: %s", model, i + 1, type(e).__name__)
    if last_err:
        raise last_err
    return ""
...
class handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        try:
            ...
            # mode=cached = 조회 전용 (프론트 자동 표시용 — 생성 비용/캡 소비 0)
            mode = (qs.get("mode", [""])[0] or "").strip()
            if mode == "cached":
                return self._send(404, {"error": "no_cache"})
