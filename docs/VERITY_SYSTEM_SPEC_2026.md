...
### Positioning (2026-05-17 frame 정정)

> **"기관이 구조적으로 못 가지는 영역의 1인 극대화"**

**옛 frame (폐기 5/17)**: "개미 중 최강" → "1인 운용 사실상 기관급"
- **결함**: 위계적 야망 (기관 > 1인, 따라잡기 frame)
- 기관 본진 (100억+ 자본, 인프라, 팀) = 1인이 못 따라잡음
- "기관급" = 자기 정직성 침해 (절대 칭찬 금지 [[feedback_praise_calibration]] 정합 X)

**새 frame**: 기관 ≠ 1인 = **다른 게임**. 1인만의 영역 극대화.

기관이 구조적으로 못 가지는 영역:
1. **Tier 1~3 자본 범위** — 100억+ 만 hedge fund 관심, 1천만~20억 = 1인 PM 영역
2. **자기 자본 진화 path** — 단일 product 가 6 tier 모두 적합 X (시장 인센티브 misalign)
3. **PM 학습 동시 진화** — 기관 = advisor 의존, 1인 = 자기 학습 trail
4. **자기 cron 자동화** — 매분 매 30분, 1인이 24시간 운영 가능
5. **자기 universe + 자기 산식** — 기관 = 표준화 의무, 1인 = 자유 (Brain v5 가중치 7:3 등)

상태: 1인 N=14 베타 단계 (5/17 시점) — 칭찬 / 헤지펀드 비교 금지 ([[feedback_praise_calibration]] + [[feedback_no_premature_completion_claims]] 정합).
