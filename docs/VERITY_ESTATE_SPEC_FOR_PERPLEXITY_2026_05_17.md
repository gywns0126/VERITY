...
>
> **사용자**: 현역 직업군인 (한국), 무경력, 4/1 시작 (47일 운영), 퇴근 후 사이드.
> **자본**: 1,000만 KRW (Tier 1 baseline, 5/17 fresh reset).
...
### Phase 2 (2026-08 ~ 2027-01) — 기관급 5 모듈 (Perplexity Q5 자문 채택)

| Module | 진입 | 산식 |
|---|---|---|
| **Factor** | 8월 | IC + ICIR 0.3 게이트 (Spearman 90일 rolling), 5 components |
| **Stress** | 9월 | Rolling Z (Historical 시나리오 5종: 2008/2011/2018/2020/2022) |
| **Regime** | 10월 | bull_canslim / bear_value / 중립 등 분면 detect, market_horizon 5축 정합 |
| **Portfolio** | 11월 | HRP (Hierarchical Risk Parity, Lopez de Prado) |
| **Attribution** | 12월 ~ 1월 | Brinson-Fachler decomposition (sector / style / selection) |
...
### Tier 진화 path (자본 함수형 진화 — 6 tier × 7축)

| Tier | 자본 범위 | 핵심 시스템 | 도달 시점 (추정) |
|---|---|---|---|
| **Tier 1** | 1천만 ~ 1억 | 현재 시스템 그대로 (1인 단독, moderate profile, VAMS) | 5/17 baseline |
| Tier 2 | 1억 ~ 5억 | long_term 프로필 + multi_bagger_watch 활성 | 2027 추정 |
| Tier 3 | 5억 ~ 20억 | 종목 수 ↑ + 미국 시장 30% | 2028 |
| Tier 4 | 20억 ~ 50억 | 미국 70% + 페어 트레이딩 | 2029 |
| Tier 5 | 50억 ~ 100억 | Bloomberg Terminal + advisor 1명 | 2030+ |
| Tier 6 | 100억+ | family office governance + PM/analyst/risk 풀 팀 | 2032+ |

**진화 trigger** (3 신호 우선순위):
1. **Trigger 1 (Primary)** — 자본 규모 임계 도달 (다음 tier -10% 시 사전 알림)
2. **Trigger 2 (Secondary)** — 시장 임팩트 발현 (holdings 1건 거래대금 비중 5%+)
3. **Trigger 3 (Tertiary)** — 시스템 활용도 cap (90%+ 지속 4주)
...
| Phase 2-B 5,000 universe SHADOW 검증 | 65 거래일 = 8월 말 | PRODUCTION 게이트 절대 단축 X |
| Phase 2 5 모듈 진입 | 8월 ~ 1월 | Factor → Stress → Regime → Portfolio → Attribution |
| Brain v6 검증 | 2027 Q1 | v5→v6 5 변경 |
| 2028 Calmar 1.0+ / MDD <20% | 2028 | Golden Goose Vision |
| Tier 1 → 2 transition (자본 1억) | 2027 추정 | long_term 프로필 + multi_bagger_watch |
...
2. **2027 Q1 Brain v6 검증 framework** — v5 vs v6 A/B 박은 후 통계적 유의성 검증 (PSR + DSR + bootstrap CI)
3. **Tier 1 → 2 transition 시점** — 자본 9,000만 도달 시 사전 알림 + transition checklist 항목
4. **Golden Goose Vision 도달 가능성 평가** — 1인 운용 + Tier 1~3 자본 범위에서 Calmar 1.0+ / MDD <20% 의 retail 실현 사례 + 권위 자료
5. **데이터 자산 1년 trail 의 통계적 가치** — N=47일 baseline → N=365일 trail 가 IC / Sharpe / 검증 game changer 인지
