...
VAMS_PROFILES = {
    ...
    "moderate": {
        ...
        "max_picks": 7,
        "max_buy_per_cycle": 5,
        # ── 손절 캡 −5.0 → −20.0 (2026-08-09, PM 승인 ·
        #    docs/PREREG_STOPLOSS_CAP_2026_08_09.md) ──────────────────────────
        # 🚨 이 값은 **안전판**이지 주 손절선이 아니다. 주 손절선은 holding 의
        #    stop_loss_pct_individual(ATR 동적)이고, engine.check_stop_loss 가
        #    max(profile, individual) 로 **더 빨리 트리거되는 쪽**을 고른다.
        #
        # 왜 바꿨나 — 실측(2026-08-09, 정본 22건 원장 재생):
        #   · 보유 21건의 ATR 분포 = 최소 −27.13% · 중앙 −14.06% · 최대 −5.88%.
        #     **−5% 보다 좁은 ATR 이 0건**이라 max() 가 항상 −5 를 골랐다.
        #   · stop_method 실측: profile_cap 15건 / individual_atr **0건**.
        #     → ATR 산출 로직 전체가 계산만 하고 결과가 쓰이지 않는 죽은 코드였다.
        #   · 그 결과 하루 변동폭 7~11% 종목이 정상 등락에 손절당했고, 손실의
        #     **100%** 가 고정 손절에서 나왔다(15건 −657,013원).
        #   · 반사실 재생(N=21, 일별 종가): 현행 −5% 가 4개 시나리오 중 꼴찌.
        #     ATR 개별은 평균 +0.11% · 승률 62%(현행 −1.77% · 38%).
        #
        # 🚨 −20 은 자의적 단일 선택이다. ATR 분포 밖으로 밀어 설계 의도(ATR 주,
        #    프로파일 안전판)를 복원하는 값이며, 재생 argmax(−10% 고정)를 고르지
        #    않은 이유는 그 차이(10,292원)가 N=21 에서 노이즈라 곡선 맞추기가 되기
        #    때문이다. **재조정하지 않는다**(RULE 7 "1회만").
        # 되돌림 조건: 20거래 내 MDD 가 −17%(현행 −8.58% 의 2배) 초과 시 즉시 원복.
        "stop_loss_pct": -20.0,
...
# 토스증권 Open API = 도입 원복 2026-06-19 (매매 외 기능 우위 없음, 기존 KIS 유지).
#   .env 의 TOSS_API_KEY/SECRET 는 2027 매매 재검토용 보존 — config 노출 안 함.
#   상세 = 메모리 project_toss_api_watch_2026_06_08.
