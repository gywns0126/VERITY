...
FCF: {fcf/1e8:+,.0f}억 {'⚠️ 현금 소진' if fcf < 0 else '✓ 현금 창출'}"""
    ...
    if pr.get("commodity_ticker"):
        cm_block = f"""[원자재] {pr.get('commodity_ticker')} | 60일 연동도 {pr.get('correlation_60d', '?')} | 마진 방어력 {pr.get('margin_safety_score', '?')}
국면: {pr.get('spread_regime', '?')} | 판가력 {pr.get('pricing_power', '?')} vs 원가 변동성 {pr.get('raw_material_volatility_score', '?')}"""
...
# ────────────────────────────────────────────────────────────
# 종합 검수 (2026-05-11 추가, project_claude_budget_guard 정합)
# ────────────────────────────────────────────────────────────
