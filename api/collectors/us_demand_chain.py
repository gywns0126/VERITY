...
CHAINS: Dict[str, Dict[str, Any]] = {
    ...
    # 🚨 2026-08-19 신설 (사전등록) — 기존 5체인은 **미변경**. 가산 추가라 기존 시계열 무손상.
    #   WHY: datacenter_infra 가 "AI 의 물리 병목 = 전력" 을 표방하는데 구성이 전력'기기'
    #     (CAT·ETN·VRT·PWR)뿐이고 **발전 축이 통째로 비어 있었다.** AI 데이터센터 전력 수요의
    #     공급측 대응이 원자력·SMR 로 몰리는데 그 관측 창이 없다.
    #     실측 계기: XE(X-energy, 2026-04-24 나스닥 상장) 분석 시 섹터 축이 조인에 0건.
    #   구성 근거: 발전사업자(XE·SMR·OKLO·NNE) + 연료·농축(LEU·CCJ·UEC) + 기존원전 EPC(BWXT).
    #     밸류체인 3단을 덮어 한 축의 개별 악재가 체인 신호를 지배하지 않게 한다.
    #   🚨 티커 8종 전부 야후 실호출로 심볼·거래소 확인(2026-08-19). 기억으로 넣지 않았다.
    #   🚨 N=0 에서 시작한다 — 기존 5체인과 관측 길이가 다르므로 비교 시 표본 길이를 병기할 것.
    "nuclear_smr": {
        "label": "원자력·SMR",
        "thesis": "AI 데이터센터 전력 수요의 공급측 대응. 국장 원전 기자재·EPC 수주 선행",
        "tickers": ["XE", "SMR", "OKLO", "LEU", "CCJ", "BWXT", "NNE", "UEC"],
        "kr_link": [("034020", "두산에너빌리티"), ("052690", "한전기술"), ("032820", "우리기술")],
    },
    "biotech": {
        "label": "바이오",
