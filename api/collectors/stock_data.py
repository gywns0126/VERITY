...
KOSDAQ_MAJOR = {
    ...
    "328130.KQ": "루닛",
    "145020.KQ": "휴젤",
    # 🚨 2026-08-24 PM 등재 — 코어는 hard_floor Rule 1~3(페니·관리종목·거래대금) 면제 경로다.
    #   에스텍은 일평균 거래대금 하한 1억원 경계에 걸려 수집 유니버스에서 빠져 있었고,
    #   그 결과 재무·분기추이·내부자·포렌식·리포트 **9개 산출물 전부에서 누락**(조인 15섹션,
    #   비교군 SAMG엔터 24 · AMG 19). 실측 최근 20봉 거래대금 평균 1.09억 / **중앙값 0.45억** —
    #   평균이 하한을 넘긴 건 8/19 하루 12.27억 스파이크 때문이라 들락날락하는 경계 종목이었다.
    #   🚨 발행↔수집 union(dart_batch_builder)은 **순환**이라 둘 다에 없는 종목을 새로 못 들인다.
    #   코어 등재가 유일한 진입로이고, PM 승인으로 스캔·추천 후보 개방까지 함께 수용한다.
    "069510.KQ": "에스텍",
}
