...
소스:
  ...
  2) 한경 컨센서스 — **2026-05-18 deprecated**:
     docs/Q2_LIVE_FETCH_VERIFICATION_20260518.md 실호출 검증 결과
     consensus.hankyung.com/robots.txt = "Disallow: /" 전체 차단.
     함수는 보존 (graceful fallback 으로 빈 list 반환), 호출 측에서 skip 권장.
  3) **한국IR협의회 (KIRS, 2026-05-18 신설)**
     - 아웃소싱: kirs.or.kr/research/research.html (NICE/서울평가 위탁, 코스닥 소형)
     - 인소싱:   kirs.or.kr/research/research22_1.html (KIRS 자체, 중·대형 보강)
     - robots.txt Allow ✓ (Q2_LIVE_FETCH_VERIFICATION 정합)
     - PDF 직링크 패턴: w4.kirs.or.kr/download/research/{YYMMDD}_{섹터}_{회사명}({종목코드})_..._{기관}.pdf

설계 원칙:
  - PDF는 URL 만 수집 — 실 다운로드는 download_report_pdf() 별도 호출
  - 종목명 → ticker 매핑은 네이버 page 의 ?code= 직접 추출 (이름 fuzzy 매칭 우회)
  - KIRS 는 회사명 () 안 6자리 종목코드 정규식 추출
...
# KIRS (한국IR협의회, 2026-05-18 신설)
KIRS_PAGES = {
    "outsourcing": "https://kirs.or.kr/research/research.html",   # NICE/서울평가 위탁
    "insourcing":  "https://kirs.or.kr/research/research22_1.html",  # KIRS 자체
}
# KIRS PDF URL 안 "회사명(123456)" 패턴에서 종목코드 추출
_KIRS_TICKER_RE = re.compile(r"\((\d{6})\)")
...
# ─── 한국IR협의회 (KIRS, 2026-05-18 신설) ─────────────────


def _extract_kirs_company_and_ticker(text: str) -> tuple:
    """\"질경이(233990)\" 또는 \"두산테스나 (131970)\" → (\"질경이\", \"233990\")."""
    m = _KIRS_TICKER_RE.search(text or "")
    if not m:
        return text.strip() if text else "", None
    ticker6 = m.group(1)
    company = (text[: m.start()] or "").strip().rstrip("(").rstrip()
    return company, ticker6


def fetch_kirs_reports(
    page_type: str = "outsourcing",
    bgn_date: Optional[str] = None,
    end_date: Optional[str] = None,
    max_pages: int = MAX_PAGES,
) -> List[Dict[str, Any]]:
    """한국IR협의회 (kirs.or.kr) 리포트 수집.

    Args:
        page_type: \"outsourcing\" (NICE/서울평가 위탁) | \"insourcing\" (KIRS 자체)
        bgn_date / end_date: YYYY-MM-DD. None 이면 필터 X (전체).

    HTML 표 구조 (table 1개, 페이지당 ~10 entries, td 5칸):
        td[0] = 회사명(종목코드) — 예 \"질경이(233990)\"
        td[1] = 제목
        td[2] = 발간기관 (outsourcing) 또는 작성자 (insourcing)
        td[3] = 발간일 (YYYY-MM-DD)
        td[4] = PDF 다운로드 a href

    검증: docs/Q2_LIVE_FETCH_VERIFICATION_20260518.md.
    robots.txt Allow ✓ / response 0.33s / captcha·로그인 없음.
    """
    url = KIRS_PAGES.get(page_type)
    if not url:
        logger.warning("[KIRS] unknown page_type=%s", page_type)
        return []
    ...
    for page in range(0, max_pages):
        try:
            r = _SESSION.get(url, params={"page": page}, headers=HEADERS, timeout=TIMEOUT)
            r.raise_for_status()
        except Exception as e:
            logger.warning("[KIRS:%s] page %d fetch 실패: %s", page_type, page, e)
            break

        soup = BeautifulSoup(r.text, "html.parser")
        tables = soup.find_all("table")
        if not tables:
            logger.info("[KIRS:%s] page %d: table 부재 — 구조 변경?", page_type, page)
            break
        ...
        for row in rows[1:]:  # row[0] = header
            ...
            company_name, ticker6 = _extract_kirs_company_and_ticker(
                tds[0].get_text(strip=True)
            )
            title = tds[1].get_text(strip=True)
            firm_or_author = tds[2].get_text(strip=True)
            date_str = tds[3].get_text(strip=True)
            # KIRS 는 이미 YYYY-MM-DD 형식이지만 안전하게 parse
            date_parsed = _parse_date(date_str) or date_str
            if not date_parsed or not re.match(r"^\d{4}-\d{2}-\d{2}$", date_parsed):
                continue
            page_oldest = date_parsed
            if bgn_date and date_parsed < bgn_date:
                continue
            if end_date and date_parsed > end_date:
                continue

            pdf_a = tds[4].find("a", href=lambda h: h and ".pdf" in h.lower())
            pdf_url = pdf_a["href"] if pdf_a else None

            key = (pdf_url, title, date_parsed)
            if key in seen:
                continue
            seen.add(key)
            ...
            entry = {
                "source": f"kirs_{page_type}",
                "ticker": ticker6,
                "ticker_yf": ticker_yf,
                "company_name": company_name,
                "title": title,
                "firm": firm_or_author if page_type == "outsourcing" else "KIRS",
                "author": firm_or_author if page_type == "insourcing" else None,
                "date": date_parsed,
                "target_price": None,
                "opinion": None,
                "pdf_url": pdf_url,
        ...
        logger.info("[KIRS:%s] page %d: +%d (oldest %s)",
                    page_type, page, page_added, page_oldest)
        if page_added == 0:
            break
        if bgn_date and page_oldest and page_oldest < bgn_date:
            break
        time.sleep(API_DELAY)
...
def scout_reports(
    ...
    company_kirs_in = fetch_kirs_reports("insourcing", bgn_date, end_date, max_pages)

    company_reports = company_naver + company_hk + company_kirs_out + company_kirs_in
    ...
    payload = {
        ...
        "stats": {
            ...
            "company_kirs_outsourcing": len(company_kirs_out),
            "company_kirs_insourcing": len(company_kirs_in),
