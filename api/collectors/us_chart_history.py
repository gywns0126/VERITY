...
멱등·재개: 한 run 안에서는 이미 받은 티커를 건너뛴다(파일 존재 자체가 체크포인트).
   🚨 **run 사이의 skip 은 기본이 아니다.** `--skip-list`(Blob 기보유 목록)는 차단으로 끊긴
   run 을 이어받을 때만 쓴다. 상시로 켜면 "이미 있는 티커는 영영 다시 안 받는" 게 되어
   월간 갱신이 사라진다 — 2026-08-09 에 `us_prices.duckdb` 에서 고친 바로 그 결함
   ("이미 있는 ticker = skip" → 날짜가 늘지 않음)과 같은 구조다.
"""
