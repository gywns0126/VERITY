...
# 🚨 2026-08-18 정정 — 종전 문구는 "가설 — 자체 산식 N<252 IC 게이트(2027-05) 미검증" 이었다.
#    그 게이트는 **폐기됐다** (`docs/VALIDATION_METHODOLOGY.md` §7-1, PM 결정 2026-08-15:
#    "실제 출력이 언제나 '더 모아라' 였다. 기다림은 결론이 아니다").
#    그런데 발행 산출물 56건 전부가 여전히 폐기된 게이트를 인용하고 있었다 —
#    죽은 전제가 코드에 남아 매 발행마다 복제되는 형태(RULE 12 가 경고한 그것).
#
#    대체 문구는 **"검증됐다" 가 아니다.** 폐기된 것은 게이트이지 미검증 사실이 아니며,
#    대체 게이트(§7-3)는 아직 PM 승인 대기다. 그래서 죽은 날짜를 빼고 현 상태만 적는다.
_VALIDATION_NOTE = "가설 — 자체 산식, 사전등록 검정 미통과 (검증 틀 v2 §7-3 승인 대기)"
