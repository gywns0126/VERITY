...
✅ 확정 2026-06-19 (Perplexity + KPMG 2025 세법개정 cross-source): ISA 한도 상향 개정은
   미통과 = 현행 유지. 고배당 선택 분리과세 특례 신설(2026~2028).

🚨 정정 2026-08-22 — 위 6/19 기재 중 **"대주주 50억→10억 환원 통과(2025-12)" 는 틀렸다.**
   실제는 **채택되지 않았고 현행 50억원이 유지**됐다. 4출처 교차 확인:
     · 기획재정부 보도자료 "주식 양도소득세 과세대상 대주주 범위, **현행 기준 유지**"
     · 대한민국 정책브리핑 "대주주 기준, 현행 '종목당 **50억 원 이상**' 유지"
     · 국회예산정책처 2025 개정세법 심의 결과 — 현행 유지로 정리
     · KDI 경제교육정보센터 (10억→50억 상향분 자료)
   10억 환원은 2025-07-31 세제개편안 **발표 단계에 그쳤고** 시행령 개정이 공포·시행된 바 없다.
   🚨 이건 국회 법률이 아니라 **기재부 소관 시행령(소득세법 §157④)** 사안이라 정부가 별도로
   개정하면 언제든 바뀐다 — 값을 고칠 때는 반드시 출처·확인일을 함께 갱신할 것.

   판정 요건 = 지분율 **OR** 시가총액(하나만 충족해도 대주주). 지분율 = 코스피 1%·코스닥 2%·
   코넥스 4%. 시가총액은 **법인 전체가 아니라 본인 보유분 평가액**(보유주식수 × 직전 사업연도
   종료일 종가). 특수관계인은 **최대주주면 합산**, 비최대주주는 본인만(2023 개정).
"""
...
KR_MAJOR_SHAREHOLDER_AMOUNT = 5_000_000_000  # 대주주 종목당 보유 평가액 기준 = 50억
#   🚨 2026-08-22 정정 (10억 → 50억). 종전 10억은 '2025-12 환원 통과' 라는 **틀린 전제**였다.
#   기재부·정책브리핑·국회예산정책처·KDI 4출처 = 현행 50억 유지. 상세 = 모듈 docstring.
#   시행령 사안이라 변동 가능 — 바꿀 때 출처·확인일 동시 갱신 의무.
