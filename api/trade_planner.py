...
def build_trade_plan_v0(stock: dict, judgment: dict) -> dict:
    ...
    if rec == "BUY":
        ...
        # Phase 1.2 (2026-05-01) — R-multiple 기반 부분 익절. magic number 1.12 폐기.
        # 1R = 진입가 - 손절가. target_1 +1R (50%), target_2 +2R (30%), target_3 트레일링 (20%).
        # exit_target (단일값) 은 deprecated — backward compat 위해 target_1.price 동기화.
        risk_per_share = price - stop_price  # 1R 거리 (KRW or USD)
        if risk_per_share <= 0:
            # 비정상 — fallback: 진입가 +10%
        ...
        exit_targets = {
            "target_1": {
                "price": target_1_price,
                "r_multiple": _cfg.R_MULTIPLE_TARGET_1,
                "exit_pct": _cfg.R_MULTIPLE_EXIT_PCT_1,
                "reason": f"+{_cfg.R_MULTIPLE_TARGET_1}R reached, lock in gains",
            },
            "target_2": {
                "price": target_2_price,
                "r_multiple": _cfg.R_MULTIPLE_TARGET_2,
                "exit_pct": _cfg.R_MULTIPLE_EXIT_PCT_2,
                "reason": f"+{_cfg.R_MULTIPLE_TARGET_2}R reached, scale out",
            },
            "target_3": {
        ...
        # Deprecated 단일 exit_target — backward compat 유지
        exit_target = {
            "price": target_1_price,
            "condition": (
                f"+1R 부분익절 (deprecated 단일값 — exit_targets 참조). "
                f"1R={int(risk_per_share):,}원"
            ),
            "deprecated": True,
        }
