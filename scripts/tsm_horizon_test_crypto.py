...
배경:
  · Liu & Tsyvinski (2021, RFS 34(6):2689-2727) 가 크립토에서 확인한 구간 = **1~4주**
  · TIDE A5 라이브 = dual-lookback **30/90일** (약 4~13주)

🚨 **2026-08-17 전제 정정.** 최초 작성 시 "90일은 문헌 검증 범위 밖" 이라고 썼는데 **틀렸다.**
   같은 라이브러리 영역 15 가 MOP(2012, JFE 104(2):228-250)를 "**1-12m lookback** ·
   TIDE A5 lookback 신호의 **직접 조상**" 이라 기재하고 있다. 30/90일 = 1~3개월이므로
   TSM 정전 논문의 검증 구간 **안쪽**이다. Liu-Tsyvinski 1~4주만 읽고 바로 위 줄의 MOP 를
   놓쳤다(RULE 13 — 표본을 읽고 전수의 확신으로 말함).

   정정된 구도 = 학술 앵커가 **둘**이고 구간이 다르다.
     MOP 2012 (다자산 TSM 정전)        → 1~12개월  ← TIDE 30/90 이 여기 들어간다
     Liu-Tsyvinski 2021 (크립토 특화)  → 1~4주
   따라서 이 검정의 질문은 "TIDE 가 문헌 밖인가" 가 아니라 **"두 앵커 중 어느 구간이
   실제로 우월한가"** 이다. 아래 실증 결과 자체는 정정 전후 동일하다.
