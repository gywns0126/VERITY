...
🚨 나는 같은 날 그 죽은 전제를 **사전등록 문서 2건에 써넣었다** — 인용 금지 대상인 폐기 토큰(N≥252)을 그대로 옮겼다.
PM 지적으로 잡혔다 — 자동 검사가 없었기 때문이다.

기존 `knowledge_surface_check` 는 kickoff 에 **섹션이 존재하는지**만 본다. 목록은 있는데
**인용 여부를 강제하지 않는다.** 그래서 목록이 장식이 됐다.

🚨 잔존 실측(2026-08-17): 코드 **9파일 15건**이 아직 폐기 토큰 `N≥252` 를 인용 중이다(인용 금지). 다음 세션이
...
def dead_tokens() -> list[str]:
    ...
    for ln in lines:
        ...
        for m in re.finditer(r"\*\*(.+?)\*\*", ln):
            ...
            #   숫자·비교기호·언더스코어를 포함한 토큰 — 폐기 목록 예시(인용 금지): N≥252, GATE_X, 2027-05 …
