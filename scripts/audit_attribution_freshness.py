...
def check_form144_units() -> List[Finding]:
    ...
    if hits:
        # 🚨 2026-08-17 P0 → WARN 강등. 진단 결과 **이미 완전히 처리된 상태**다:
        #   ① 검출 = 빌더 자체 함수(`_flag_implied_price_outliers`)가 잡는다
        #   ② 보존 = 삭제하지 않고 `value_suspect` 에 사유·배수를 달아 남긴다
        #      (빌더 144행 "근거 없이 지우느니 남긴다")
        #   ③ 격리 = **합계에서 제외**한다 (빌더 255~257행 — 하나가 총액을 지배하므로)
        #   ④ 개별 `value_usd` 를 읽는 소비처 = 전수 grep **0건**. 발행물
        #      `us_form144_public.json` 자체가 존재하지 않는다.
        #   즉 우리 파이프라인 결함이 아니라 **SEC 제출인 기입 오류라는 사실**이고,
        #   그 사실은 이미 라벨링·격리돼 어떤 판단에도 도달하지 않는다.
        #
        #   P0 정의는 "값이 **조용히** 틀린 상태" 다. 이건 조용하지 않다 —
        #   라벨이 붙어 있고 합계에서 빠져 있다. P0 로 두면 감사 워크플로가 고칠 수 없는
        #   상류 사실 때문에 영구히 exit 1 이고, 그러면 사람이 감사를 무시하게 된다.
        #   워크플로 주석이 경계하는 "감사가 초록불인데 값이 틀린" 의 정확한 역상이다.
        #   🚨 개별 값을 읽는 소비처가 생기면 즉시 P0 로 되돌릴 것 (④가 전제다).
        return [Finding("WARN", "f144-unit",
                        f"주당 환산 이상치 {len(hits)}건 (제출인 기입 오류 — 라벨링+합계 제외 완료)",
